...
DECLARE
  ...
  v_cid uuid; v_vid uuid; v_oid_bunch uuid;
...
BEGIN
  ...
  SELECT count(*) INTO v_orderable0 FROM public.v_marche_listing_truth WHERE is_orderable;
  ...
  SELECT merchant_platform_fee_bps INTO v_fee0 FROM public.finance_policies
   WHERE mission_type='marche' AND enabled AND effective_from <= now()
  ...
  VALUES (v_adm, 'god_admin', 'active');
  INSERT INTO public.user_roles(user_id, role) VALUES (v_mer,'merchant'), (v_drv,'driver');

  -- ================= B. HIERARCHY RUNTIME =================
  v_err := public._qa_r6_err('authenticated', v_adm,
    $q$SELECT public.marche_staple_commodity_upsert('{"code":"qa_r6_grain","category_code":"cereals_starch","name_fr":"QA R6 Grain","aliases":["qa r6 grain"],"unit_family":"mass","sort_order":900}'::jsonb)$q$);
  ...
  v_err := public._qa_r6_err('authenticated', v_adm,
    $q$SELECT public.marche_staple_variant_upsert('{"commodity_code":"qa_r6_grain","code":"general","name_fr":"QA R6 General","is_default":true}'::jsonb)$q$);
  ...
  -- ================= L. RAIL SEPARATION =================
  r := r || public._qa_s13_ok('N4R6.L1 zero staple identities leaked into marketplace_listings',
  ...
  SELECT count(*) INTO v_n FROM public.v_marche_listing_truth WHERE is_orderable;
  ...
  SELECT count(*) INTO v_n FROM public.marche_staple_commodities WHERE code LIKE 'qa\_r6%';
