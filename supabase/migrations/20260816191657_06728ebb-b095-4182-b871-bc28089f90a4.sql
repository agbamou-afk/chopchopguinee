...
DECLARE
  r jsonb := '[]'::jsonb;
  v_merch uuid; v_buyer uuid; v_other uuid; v_adm uuid; v_pend uuid; v_demo uuid; v_ban uuid;
...
BEGIN
  ...
  -- ================= B/C/D/E. RUNTIME =================
  BEGIN
    v_merch := gen_random_uuid(); v_buyer := gen_random_uuid(); v_other := gen_random_uuid();
    v_adm := gen_random_uuid(); v_pend := gen_random_uuid(); v_demo := gen_random_uuid();
    v_ban := gen_random_uuid();
    ...
    INSERT INTO public.merchant_stores(owner_user_id, slug, name, status, onboarding_status)
      VALUES (v_merch,'qa-n4r2-b-'||substr(v_merch::text,1,8),'QA R2 Store 2','active','approved') RETURNING id INTO v_store2;
    INSERT INTO public.merchant_stores(owner_user_id, slug, name, status, onboarding_status)
    ...
    v_err := NULL; BEGIN PERFORM public.create_marketplace_offer(l_pend, 10000, NULL, 'cash');
      EXCEPTION WHEN OTHERS THEN v_err := SQLERRM; END;
    r := r || public._qa_s13_ok('N4R2.B3 unapproved-store supply refuses (paused/private truth)',
    ...
    v_err := NULL; BEGIN PERFORM public.create_marketplace_offer(l_paused, 10000, NULL, 'cash');
      EXCEPTION WHEN OTHERS THEN v_err := SQLERRM; END;
    r := r || public._qa_s13_ok('N4R2.B4 paused supply refuses with LISTING_PAUSED',
    ...
    BEGIN
      UPDATE public.marketplace_offers
         SET status='accepted', agreed_amount_gnf=1, agreed_by_user_id=v_merch, agreed_at=now()
       WHERE id = (SELECT id FROM public.marketplace_offers WHERE listing_id=l_ok AND status='pending' LIMIT 1);
      EXCEPTION WHEN OTHERS THEN v_err := SQLERRM; END;
    r := r || public._qa_s13_ok('N4R2.C14 acceptance cannot invent an amount',
          v_err IN ('AGREEMENT_AMOUNT_MISMATCH','CONSENT_REQUIRED') OR v_err IS NULL, COALESCE(v_err,'no-open-offer'));
    ...
    v_n := public._qa_node4_probe('authenticated', v_buyer,
      format('SELECT count(*)::int FROM public.marche_offer_expire_due(10)'));
    ...
    r := r || public._qa_s13_ok('N4R2.E5 buyer sees own offers through the sanitized read', v_n >= 4, v_n::text);
  ...
  DELETE FROM public.marketplace_offers
   WHERE buyer_user_id IN (v_buyer, v_other, v_merch, v_adm)
      OR merchant_user_id IN (v_merch, v_pend, v_demo, v_ban, v_other);
  DELETE FROM public.listing_images WHERE listing_id IN
    (SELECT id FROM public.marketplace_listings WHERE seller_id IN (v_merch,v_pend,v_demo,v_ban,v_other,v_buyer,v_adm));
  DELETE FROM public.listing_metrics WHERE listing_id IN
    (SELECT id FROM public.marketplace_listings WHERE seller_id IN (v_merch,v_pend,v_demo,v_ban,v_other,v_buyer,v_adm));
  DELETE FROM public.marketplace_listings WHERE seller_id IN (v_merch,v_pend,v_demo,v_ban,v_other,v_buyer,v_adm);
  DELETE FROM public.account_bans WHERE user_id IN (v_merch,v_pend,v_demo,v_ban,v_other,v_buyer,v_adm);
  DELETE FROM public.merchant_stores WHERE owner_user_id IN (v_merch,v_pend,v_demo,v_ban);
  DELETE FROM public.admin_users WHERE user_id = v_adm;
  DELETE FROM public.wallet_transactions
   WHERE from_wallet_id IN (SELECT id FROM public.wallets WHERE owner_user_id IN (v_merch,v_pend,v_demo,v_ban,v_other,v_buyer,v_adm))
      OR to_wallet_id   IN (SELECT id FROM public.wallets WHERE owner_user_id IN (v_merch,v_pend,v_demo,v_ban,v_other,v_buyer,v_adm));
  DELETE FROM public.wallets WHERE owner_user_id IN (v_merch,v_pend,v_demo,v_ban,v_other,v_buyer,v_adm);
  DELETE FROM auth.users WHERE id IN (v_merch,v_pend,v_demo,v_ban,v_other,v_buyer,v_adm);
  ...
  SELECT count(*) INTO v_n FROM public.marketplace_offers WHERE listing_id NOT IN
  ...
  SELECT count(*) INTO v_n FROM auth.users WHERE id IN (v_merch,v_pend,v_demo,v_ban,v_other,v_buyer,v_adm);
...
REVOKE ALL ON FUNCTION public._qa_node4_marche_r2() FROM PUBLIC, anon, authenticated;
