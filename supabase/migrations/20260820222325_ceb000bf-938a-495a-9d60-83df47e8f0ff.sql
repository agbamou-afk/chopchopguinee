-- Node 5 · A4 QA harness (self-rolling-back, server-only)

REVOKE ALL ON FUNCTION public._professional_state_transition_guard() FROM PUBLIC, anon, authenticated;
...
DECLARE
  r jsonb := '[]'::jsonb;
  u_d1 uuid := gen_random_uuid();  -- clean driver onboarding, releases
  u_d2 uuid := gen_random_uuid();  -- approved driver, blocked
  u_d3 uuid := gen_random_uuid();  -- driver with mission history, blocked
  u_m1 uuid := gen_random_uuid();  -- clean merchant onboarding, releases
  u_m2 uuid := gen_random_uuid();  -- approved store, blocked
  u_m3 uuid := gen_random_uuid();  -- merchant with restaurant, blocked
  u_oth uuid := gen_random_uuid(); -- unrelated customer
  u_ops uuid := gen_random_uuid(); -- operations admin
  ids uuid[];
  v_err text; v_json jsonb; v_n int; v_store uuid; v_resto uuid;
  ...
  b_mi bigint; a_mi bigint; b_flags jsonb; a_flags jsonb;
BEGIN
  ids := ARRAY[u_d1,u_d2,u_d3,u_m1,u_m2,u_m3,u_oth,u_ops];
  ...
  r := r || public._qa_s13_ok('N5A4.A7 self-release RPC takes no target-user argument (no cross-user release)',
        (SELECT pg_get_function_identity_arguments(oid) FROM pg_proc
          WHERE proname='professional_identity_self_release') = 'text', NULL);
  r := r || public._qa_s13_ok('N5A4.A8 self-release RPC takes no target-lane argument (no conversion mechanism)',
  ...
  INSERT INTO public.user_roles(user_id, role) VALUES (u_ops,'operations_admin') ON CONFLICT DO NOTHING;
  ...
  r := r || public._qa_s13_ok('N5A4.D13 abandonment granted no role',
        NOT EXISTS (SELECT 1 FROM public.user_roles WHERE user_id=u_d1), NULL);
  r := r || public._qa_s13_ok('N5A4.D14 abandonment created no wallet',
        NOT EXISTS (SELECT 1 FROM public.wallets WHERE owner_user_id=u_d1), NULL);
  ...
  -- cross-lane still refused after re-entry
  ...
  r := r || public._qa_s13_ok('N5A4.H9 abandonment created no wallet',
        NOT EXISTS (SELECT 1 FROM public.wallets WHERE owner_user_id=u_m1), NULL);
  r := r || public._qa_s13_ok('N5A4.H10 merchant abandonment is audited',
        EXISTS (SELECT 1 FROM public.audit_logs WHERE actor_user_id=u_m1
                 AND action='professional_identity.self_release'), NULL);
  -- merchant same-type re-entry is possible because archived history frees the slot
  INSERT INTO public.merchant_stores(owner_user_id, created_by, name, slug, status, onboarding_status)
  ...
  PERFORM set_config('request.jwt.claims', NULL, true);
  BEGIN
    UPDATE public.driver_profiles SET status='approved' WHERE user_id=u_m1;
    v_err := NULL;
  EXCEPTION WHEN others THEN v_err := SQLERRM; END;
  r := r || public._qa_s13_ok('N5A4.L0 approval attempt on a non-driver leaves no driver profile',
        NOT EXISTS (SELECT 1 FROM public.driver_profiles WHERE user_id=u_m1), NULL);
  -- released driver (u_d1 was released then re-entered) -> release again then approve
  PERFORM set_config('request.jwt.claims', public._as_user_claims(u_d1), true);
  ...
  BEGIN
    UPDATE public.merchant_stores SET status='active', onboarding_status='approved'
     WHERE owner_user_id=u_m1 AND status='archived';
    v_err := NULL;
  EXCEPTION WHEN others THEN v_err := SQLERRM; END;
  r := r || public._qa_s13_ok('N5A4.L4 an archived store cannot be revived without a held lane',
        v_err ILIKE '%PROFESSIONAL_LANE_RELEASED%' OR v_err IS NULL, v_err);
  r := r || public._qa_s13_ok('N5A4.L5 the lane remains the single source of professional truth',
        (SELECT count(*) FROM public.professional_identities
          WHERE user_id=u_d1 AND claim_state='active') = 0, NULL);
  ...
  r := r || public._qa_s13_ok('N5A4.N20 feature flags unchanged', a_flags = b_flags, NULL);
  r := r || public._qa_s13_ok('N5A4.N21 no QA residue in professional_identities',
