-- Fix: trusted server contexts (no JWT) must not be treated as an untrusted merchant.
...
BEGIN
  ...
  BEGIN
    ...
    -- a *published* restaurant that has no menu at all
    INSERT INTO public.food_restaurants(owner_user_id, slug, name, status, verification_state,
    ...
    EXCEPTION WHEN OTHERS THEN v_err := SQLERRM; END;
    r := r || public._qa_s13_ok('P3.8 stranger cannot publish someone else’s restaurant',
          v_err LIKE '%NOT_AUTHORIZED%', v_err);
    ...
    SELECT count(*) INTO v_n FROM public.audit_logs
     WHERE module='repas' AND action='restaurant_publish' AND target_id=v_resto;
    r := r || public._qa_s13_ok('P4.2 publication is audited exactly once', v_n = 1, v_n::text);
    SELECT count(*) INTO v_n FROM public.audit_logs
     WHERE module='repas' AND target_id=v_resto AND actor_user_id=v_admin;
    ...
    r := r || public._qa_s13_ok('P5.7 Chop Pay capability is reported off until staff approve it',
    ...
    -- ===== P6. ZERO-MENU SUPPLY NEVER INFLATES THE MARKETPLACE =====
    ...
    INSERT INTO public.food_menu_items(restaurant_id,name,price_gnf,is_available)
      VALUES (v_resto2,'QA R8 Plat caché',10000,true);
    ...
    SELECT count(*) INTO v_n FROM public.audit_logs
     WHERE module='repas' AND action='restaurant_suspend' AND target_id=v_resto;
    ...
    SELECT count(*) INTO v_n FROM public.audit_logs WHERE module='repas' AND target_id=v_resto;
