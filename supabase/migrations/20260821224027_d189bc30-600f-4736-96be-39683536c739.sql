...
BEGIN
  ...
  r := r || public._qa_s13_ok('N5DL.B2 the held fixture really holds funds',
        (SELECT held_gnf FROM public.wallets WHERE owner_user_id=u_held AND party_type='client') = 3000, NULL);
  ...
  r := r || public._qa_s13_ok('N5DL.D3 the blocked debtor keeps its balance untouched',
        (SELECT balance_gnf FROM public.wallets WHERE owner_user_id=u_debt AND party_type='driver') = 4000, NULL);
