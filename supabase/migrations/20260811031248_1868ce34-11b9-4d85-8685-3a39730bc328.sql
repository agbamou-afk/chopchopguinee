    ...
    BEGIN
      ...
      SELECT w.owner_user_id, w.id, w.balance_gnf INTO v_d, v_w_d, b_d
        FROM public.wallets w WHERE w.party_type='driver' AND w.status='active'
          AND w.owner_user_id NOT IN (v_c1, v_c2)
        ORDER BY w.created_at LIMIT 1;
      ...
      DECLARE v_txd uuid;
      BEGIN
        SELECT id INTO v_txd FROM public.wallet_topup_om_credit(v_e1, v_t2);
        r := r || format('%s|D4 consumed event returns original credit, never a second one|tx_same=%s',
              CASE WHEN v_txd = v_tx
                    AND (SELECT balance_gnf FROM public.wallets WHERE id=v_w_c2)=b_c2
                    AND (SELECT transaction_id FROM public.topup_requests WHERE id=v_t2) IS NULL
                   THEN 'PASS' ELSE 'FAIL' END, (v_txd = v_tx));
      EXCEPTION WHEN OTHERS THEN
        r := r || format('PASS|D4 consumed event cannot credit a second customer|%s', SQLERRM);
      END;
