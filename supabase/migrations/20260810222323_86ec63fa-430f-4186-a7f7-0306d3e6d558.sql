...
DECLARE
  ...
  v_n bigint; v_b bigint; v_b2 bigint; v_b3 bigint; v_c0 bigint; v_mer0 bigint;
BEGIN
  ...
  BEGIN
    ...
    r := r || public._qa_s5_ok('L3 close_no_value creates no platform revenue', v_b = v_m0, v_b::text);
    SELECT state INTO v_txt FROM public.chop_pay_order_runtime WHERE source_id=v_fo;
    r := r || public._qa_s5_ok('L4 runtime leaves the disputed state', v_txt <> 'disputed', v_txt);
    ...
    r := r || public._qa_s5_ok('L5 no encumbrance remains open on the order', v_n=0, v_n::text);
    v_j := public.admin_chop_pay_dispute_resolve('repas', v_fo, 'close_no_value', 'qa_replay');
    SELECT balance_gnf INTO v_b FROM public.wallets WHERE party_type='master' LIMIT 1;
    r := r || public._qa_s5_ok('L6 close_no_value replay zero movement', v_b = v_m0, v_j::text);
    ...
    SELECT balance_gnf INTO v_mer0 FROM public.wallets WHERE owner_user_id=v_own AND party_type='merchant';
    r := r || public._qa_s5_ok('M3 Marche merchant captured 150000', v_mer0 = 150000, v_mer0::text);
    SELECT balance_gnf INTO v_m0 FROM public.wallets WHERE party_type='master' LIMIT 1;
    PERFORM set_config('request.jwt.claims', public._as_user_claims(v_drv), true);
    ...
    -- unknown / absent tender must be denied
    INSERT INTO public.marketplace_offers(listing_id,merchant_store_id,buyer_user_id,merchant_user_id,
      offer_amount_gnf,status,metadata)
    ...
    PERFORM set_config('request.jwt.claims', public._as_user_claims(v_cust), true);
    PERFORM public.chop_pay_authorize_order('repas', v_fo);
    UPDATE public.finance_policies
       SET transaction_fee_bps = 500, collateral_pct_bps = 9000,
           cancel_before_dispatch_bps = 5000, cancel_after_dispatch_bps = 6000
     WHERE mission_type = 'repas';
    SELECT order_total_gnf, platform_fee_gnf INTO v_b, v_b2 FROM public.chop_pay_order_runtime WHERE source_id=v_fo;
    r := r || public._qa_s5_ok('N1 authorized order economics stay frozen after a policy change',
  ...
  ------------------------------------------------------------------
  -- P. PRIVILEGE MATRIX (no fixtures required)
  ------------------------------------------------------------------
  ...
  SELECT count(*) INTO v_n FROM pg_proc p JOIN pg_namespace n ON n.oid=p.pronamespace
   WHERE n.nspname='public'
     AND p.proname IN ('chop_pay_customer_hold_place','chop_pay_customer_capture','chop_pay_customer_refund',
                       'wallet_internal_transfer','om_auto_match','wallet_topup_om_credit',
                       'ride_accept','ride_dispatch')
     AND (has_function_privilege('anon', p.oid, 'EXECUTE')
       OR has_function_privilege('authenticated', p.oid, 'EXECUTE'));
  ...
  r := r || public._qa_s5_ok('Z3 om_topup_enabled is the only enabled flag', v_n=0, v_n::text);
  SELECT transaction_fee_bps INTO v_b FROM public.finance_policies
   WHERE mission_type='repas' ORDER BY effective_from DESC LIMIT 1;
  r := r || public._qa_s5_ok('Z4 finance policy rows unchanged after rollback', v_b=100, v_b::text);
...
REVOKE ALL ON FUNCTION public._qa_s5_run3() FROM PUBLIC;

DO $$ BEGIN INSERT INTO public._qa_s5_results(report) SELECT public._qa_s5_run3(); END $$;