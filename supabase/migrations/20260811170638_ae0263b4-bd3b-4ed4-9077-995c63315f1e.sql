...
DECLARE
  ...
  v_pcode text; v_dcode text; v_claim uuid; v_auth bigint;
BEGIN
  ...
  BEGIN
    ...
    r := r || public._qa_s13_ok('F4.1 self-service cancellation refused once the courier has custody',
      v_err <> 'NO_ERROR'
      OR COALESCE(v_res->>'error','') = 'CUSTODY_ESTABLISHED_CLAIM_REQUIRED',
    ...
    EXCEPTION WHEN OTHERS THEN v_err := SQLERRM; v_res := '{}'::jsonb; END;
    SELECT id, authorized_gnf INTO v_claim, v_auth FROM public.claims_reserves
     WHERE source_id = v_pkg ORDER BY created_at DESC LIMIT 1;
    r := r || public._qa_s13_ok('F6.1 a claim allocates a reserve within the frozen exposure',
      v_err = 'NO_ERROR' AND v_claim IS NOT NULL AND v_auth <= v_rt.claims_exposure_gnf,
      format('%s authorized=%s exposure=%s', v_err, v_auth, v_rt.claims_exposure_gnf));

    PERFORM set_config('request.jwt.claims', public._as_user_claims(v_cust), true);
    BEGIN v_res := public.claims_reserve_resolve(v_claim, 1000, 'qa non admin'); v_err := 'NO_ERROR';
    EXCEPTION WHEN OTHERS THEN v_err := SQLERRM; END;
    r := r || public._qa_s13_ok('F6.2 only a God Admin can adjudicate a claim', v_err <> 'NO_ERROR', v_err);
    ...
    BEGIN v_res := public.claims_reserve_resolve(v_claim, v_auth + 5000000, 'qa adjudication test');
      v_err := 'NO_ERROR';
    EXCEPTION WHEN OTHERS THEN v_err := SQLERRM; v_res := '{}'::jsonb; END;
    r := r || public._qa_s13_ok('F6.3 a claim can never pay more than the authorized reserve',
