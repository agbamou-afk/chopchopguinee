...
DECLARE
  ...
  base_obs int; base_rep int; base_ord int; base_pol int; base_lst int; base_req int;
  ev jsonb; ev2 jsonb; j jsonb; pol jsonb;
  v_n bigint; v_err text;
  d1 jsonb; d2 jsonb; pa int; pe int;
  fdef text;
BEGIN
  ...
  BEGIN
    ...
    SELECT o INTO pa FROM (SELECT y.id, row_number() OVER () o FROM public.marche_listings_discover(
      'QA N410', NULL, NULL, 'recommended', 60, 0, 9.5370, -13.6785) y) t WHERE t.id = la;
    SELECT o INTO pe FROM (SELECT y.id, row_number() OVER () o FROM public.marche_listings_discover(
      'QA N410', NULL, NULL, 'recommended', 60, 0, 9.5370, -13.6785) y) t WHERE t.id = le;
    r := r || public._qa_s13_ok('N4R10.I5 between two identical listings the cheaper one ranks higher',
    ...
    WITH req AS (
      ...
      SELECT v_buy, 100000, 'available', 'observed_procurement', 50000, 'medium', 5,
             1, 1, 'qa-n410-req'||g, 'qa-n410-req'||g
        FROM generate_series(1,3) g
      RETURNING id, client_request_id
    )
    INSERT INTO public.marche_procurement_missions(request_id, buyer_user_id, shopper_user_id,
      state, assigned_at, shopping_started_at, purchase_submitted_at, completed_at, cancelled_at)
    SELECT req.id, v_buy, v_drv,
           CASE WHEN req.client_request_id = 'qa-n410-req3' THEN 'cancelled' ELSE 'completed' END,
           now() - interval '90 min',
           CASE WHEN req.client_request_id = 'qa-n410-req3' THEN NULL ELSE now() - interval '60 min' END,
           CASE WHEN req.client_request_id = 'qa-n410-req3' THEN NULL ELSE now() - interval '40 min' END,
           CASE WHEN req.client_request_id = 'qa-n410-req3' THEN NULL ELSE now() - interval '30 min' END,
           CASE WHEN req.client_request_id = 'qa-n410-req3' THEN now() ELSE NULL END
      FROM req;
  ...
  r := r || public._qa_s13_ok('N4R10.N6 zero procurement request residue', v_n = base_req, format('%s->%s', base_req, v_n));
