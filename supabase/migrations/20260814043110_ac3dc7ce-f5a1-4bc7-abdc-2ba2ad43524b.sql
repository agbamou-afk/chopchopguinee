...
DECLARE
  r jsonb := '[]'::jsonb;
  v_cust uuid; v_cust2 uuid; v_cust3 uuid; v_merch uuid; v_drv uuid; v_drv2 uuid;
  ...
  v_mission uuid; v_mission4 uuid;
...
BEGIN
  ...
  -- MC2/MC3 must be part of the canonical source, not only observable behaviour
  ...
  BEGIN
    ...
    -- flags are still exactly as production has them at this point.
    ...
    -- enable the cash rail INSIDE the rolled-back fixture only
    ...
    -- rail flipped OFF after commitment: engagement must still fail closed
    ...
    -- MC4b: Chop Pay customer cancellation routes to the Slice 5 path
    PERFORM set_config('request.jwt.claims', public._as_user_claims(v_cust2), true);
    ...
    -- merchant reject on a fresh Chop Pay order releases the hold
    PERFORM set_config('request.jwt.claims', public._as_user_claims(v_cust2), true);
    v_res := public.repas_order_create(v_resto,
        jsonb_build_array(jsonb_build_object('menu_item_id',v_item2,'qty',1)),
    ...
    -- courier engagement through the real mission path
    ...
    -- merchant accept -> merchandise capture
    PERFORM set_config('request.jwt.claims', public._as_user_claims(v_merch), true);
    ...
    -- custody + delivery through the real courier RPCs
    PERFORM set_config('request.jwt.claims', public._as_user_claims(v_drv2), true);
    ...
    -- idempotent re-delivery must not double settle
  ...
  SELECT count(*) INTO v_n FROM public.merchant_payables p
   WHERE p.source_module='repas'
     AND EXISTS (SELECT 1 FROM public.food_restaurants fr
                  WHERE fr.slug LIKE 'qa-n3-%' );
  r := r || public._qa_s13_ok('Z3.8 no merchant payable residue', v_n = 0, v_n::text);
