...
BEGIN
  ...
  VALUES (u_d, 'approved', (SELECT enumlabel::text FROM pg_enum e JOIN pg_type t ON t.oid=e.enumtypid WHERE t.typname='vehicle_type' ORDER BY e.enumsortorder LIMIT 1)::vehicle_type, now());
  ...
  -- customer submits OM code (real entrypoint)
  ...
  -- inbound provider event (as service/trusted path)
  ...
  -- E3 real auto-match path credits
  v_match := to_jsonb(public.om_auto_match(v_event));
  r := r || jsonb_build_object('E3_auto_match_credited', (v_match->>'status') = 'credited');
  ...
  -- E7/E8 eligibility recovers automatically
  PERFORM set_config('request.jwt.claims', json_build_object('sub',u_d::text,'role','authenticated')::text, true);
  v_elig := to_jsonb(public.driver_financial_eligibility('ride', 100000));
  r := r || jsonb_build_object(
  ...
  -- E10 self-only signature / non-driver denied
  ...
  -- E11 ordinary customer top-up still targets client
...
REVOKE ALL ON FUNCTION public._qa_s3d_run() FROM PUBLIC, anon, authenticated;