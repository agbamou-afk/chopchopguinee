...
BEGIN
  ...
  BEGIN
    ...
    v_ex := public.finance_treasury_exceptions();
    ...
    ELSE
      v_dd := to_jsonb(public.finance_treasury_drilldown(v_ex_pick->>'code', 50));
    ...
    ---------------------------------------------------------------- A9-A12: live movements reflected
    ...
    r := r || public._qa_s13_ok('C1 a duplicated ride acceptance creates no second commission hold',
      (SELECT count(*) FROM (SELECT public.ride_accept(v_ride)) z) = 1
      AND (SELECT count(*) FROM public.mission_financial_holds WHERE source_id=v_ride AND kind='commission') = 1,
      v_n::text);
    ...
    SELECT count(*) INTO v_n FROM public.driver_group_commissions WHERE ride_id = v_ride;
    ...
    ---------------------------------------------------------------- merchant settlement reservation + manual OM replay
    ...
    r := r || public._qa_s13_ok('C5 an attested manual Orange Money settlement debits the merchant exactly once',
      COALESCE(v_res->>'status','') = 'settled' AND v_b0 - v_b1 = 100000, format('%s %s->%s', v_res->>'status', v_b0, v_b1));
    v_res := public.finance_confirm_manual_om_payout(v_ord, 'OM-P7-'||v_sfx||'-A', true, NULL);
    SELECT balance_gnf INTO v_b0 FROM public.wallets WHERE owner_user_id=v_m1 AND party_type='merchant';
    ...
    PERFORM public.package_verify_pickup(v_pkg, '000000');
    ...
    SELECT count(*) INTO v_n FROM public.wallet_transactions
     WHERE metadata->>'topup_request_id' = v_topup::text OR reference LIKE '%'||v_topup::text||'%';
    r := r || public._qa_s13_ok('C16 exactly one wallet transaction exists for the credited top-up',
      v_n <= 1, v_n::text);

    -- sandbox provider event carrying the same reference must never credit production
    ...
    PERFORM set_config('request.jwt.claims','',true);
    SELECT balance_gnf INTO v_b0 FROM public.wallets WHERE owner_user_id=v_c2 AND party_type='client';
    v_res := public.om_auto_match(v_evt);
    SELECT balance_gnf INTO v_b1 FROM public.wallets WHERE owner_user_id=v_c2 AND party_type='client';
    ...
    r := r || public._qa_s13_ok('D2 the environment mismatch is refused explicitly, not silently remapped',
      COALESCE(v_res->>'status','') IN ('awaiting_customer_code','needs_review','rejected'), v_res::text);
    ...
    v_res := public._qa_s13_rls_probe('authenticated', v_c2, 'photos', 'package-evidence',
             v_c1::text||'/'||v_qid::text||'/p1.jpg');
    r := r || public._qa_s13_ok('B14 another customer cannot see an Envoyer evidence photo record',
      COALESCE((v_res->>'count')::bigint, -1) = 0, v_res::text);
    v_res := public._qa_s13_rls_probe('anon', NULL, 'select', 'package-evidence',
             v_c1::text||'/'||v_qid::text||'/p1.jpg');
    r := r || public._qa_s13_ok('B15 an anonymous caller cannot read an Envoyer evidence object',
      COALESCE((v_res->>'count')::bigint, -1) = 0, v_res::text);
    ...
    SELECT count(*) INTO v_n FROM jsonb_array_elements(public.finance_treasury_exceptions()) x
     WHERE x->>'code' IN ('LEDGER_GLOBAL_IMBALANCE','LEDGER_JOURNAL_IMBALANCE');
