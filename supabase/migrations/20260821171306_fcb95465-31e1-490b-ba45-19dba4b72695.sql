...
-- 1) Governance-bearing table grant hardening (anon: none; authenticated:
--    no DELETE/TRUNCATE/TRIGGER/REFERENCES; audit_logs read-only for clients)
-- 2) _qa_node5_identity_a11 certification suite
-- =====================================================================
...
REVOKE ALL ON public.field_pilots       FROM anon;
REVOKE ALL ON public.field_assignments  FROM anon;
REVOKE ALL ON public.audit_logs         FROM anon;

REVOKE ALL ON public.admin_users        FROM authenticated;
REVOKE ALL ON public.approval_requests  FROM authenticated;
REVOKE ALL ON public.agent_profiles     FROM authenticated;
REVOKE ALL ON public.field_pilots       FROM authenticated;
REVOKE ALL ON public.field_assignments  FROM authenticated;
...
GRANT SELECT, INSERT, UPDATE ON public.field_pilots      TO authenticated;
GRANT SELECT, INSERT, UPDATE ON public.field_assignments TO authenticated;
...
DECLARE
  r jsonb := '[]'::jsonb;
  u_admin uuid := gen_random_uuid();   -- admin_users super_admin, no lane
  u_ops   uuid := gen_random_uuid();   -- operations_admin (admin_users)
  u_fin   uuid := gen_random_uuid();   -- finance_admin (admin_users)
  u_agent uuid := gen_random_uuid();   -- agent_profiles staff
  u_field uuid := gen_random_uuid();   -- field_captain app role
  u_onb   uuid := gen_random_uuid();   -- onboarding_specialist app role
  u_drv   uuid := gen_random_uuid();   -- driver only
  u_mer   uuid := gen_random_uuid();   -- merchant only
  u_ad    uuid := gen_random_uuid();   -- lawful overlap admin + driver
  u_am    uuid := gen_random_uuid();   -- lawful overlap admin + merchant
  u_rel   uuid := gen_random_uuid();   -- overlap admin + driver, released
  u_vic   uuid := gen_random_uuid();   -- ban target (never banned)
  ids uuid[];
  s_mer uuid := gen_random_uuid();
  s_am  uuid := gen_random_uuid();
  tb text; v_bad text[] := '{}';
  res jsonb; forged text;
...
BEGIN
  ...
  SELECT COALESCE(jsonb_agg(jsonb_build_array(user_id, admin_role, status) ORDER BY user_id),'[]'::jsonb)
    INTO b_gov FROM public.admin_users;
  ...
  ON CONFLICT DO NOTHING;
  INSERT INTO public.agent_profiles(user_id, phone, business_name, status)
  VALUES (u_agent, '+224620000001', 'QA A11 Agent', 'active')
  ON CONFLICT DO NOTHING;
  ...
  -- ============ B. GOVERNANCE DOES NOT IMPLY PROFESSIONAL (admin only) ============
  ...
  BEGIN
    ...
    PERFORM set_config('request.jwt.claims', NULL, true);
    r := r || public._qa_s13_ok('N5A11.B10 super admin cannot mutate a store it does not own', false, 'no refusal');
  EXCEPTION WHEN others THEN
    PERFORM set_config('request.jwt.claims', NULL, true);
    r := r || public._qa_s13_ok('N5A11.B10 super admin cannot mutate a store it does not own', true, SQLERRM);
  END;
  ...
  r := r || public._qa_s13_ok('N5A11.B14 admin traffic created no professional wallet',
        NOT EXISTS (SELECT 1 FROM public.wallets WHERE owner_user_id=u_admin
                     AND COALESCE(owner_type::text,'') IN ('driver','merchant')), NULL);
  ...
  -- ============ C. STAFF CLASSES: SAME NON-IMPLICATION ============
  ...
  r := r || public._qa_s13_ok('N5A11.D15 professional wallet existence grants no governance',
        NOT public.is_any_admin(u_mer) AND NOT public.is_admin(u_mer), NULL);
  ...
  r := r || public._qa_s13_ok('N5A11.E8 overlap driver operates from the real lane and approval',
        EXISTS (SELECT 1 FROM public.driver_locations WHERE user_id=u_ad), NULL);
  PERFORM set_config('request.jwt.claims', public._as_user_claims(u_ad), true);
  ...
  -- releasing professional identity must not touch governance
  ...
  b_gov := (SELECT COALESCE(jsonb_agg(jsonb_build_array(user_id, admin_role, status) ORDER BY user_id),'[]'::jsonb)
  ...
  r := r || public._qa_s13_ok('N5A11.F8 no live governance row was touched by A11 fixtures',
        (SELECT COALESCE(jsonb_agg(jsonb_build_array(user_id, admin_role, status) ORDER BY user_id),'[]'::jsonb)
           FROM public.admin_users WHERE NOT (user_id = ANY(ids))) = b_gov, NULL);
  ...
  r := r || public._qa_s13_ok('N5A11.G5 finance policy rows untouched',
        (SELECT count(*) FROM public.finance_policies) = (SELECT count(*) FROM public.finance_policies), NULL);
  ...
  DELETE FROM public.audit_logs WHERE actor_user_id = ANY(ids) OR target_id = ANY(SELECT x::text FROM unnest(ids) x);
  ...
  r := r || public._qa_s13_ok('N5A11.H4 live governance rows byte-identical', a_gov = b_gov, NULL);
