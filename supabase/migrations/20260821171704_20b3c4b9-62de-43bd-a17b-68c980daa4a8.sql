...
BEGIN
  ...
  r := r || public._qa_s13_ok('N5A11.H4 live governance rows byte-identical',
        a_gov = (SELECT COALESCE(jsonb_agg(jsonb_build_array(user_id, admin_role, status) ORDER BY user_id),'[]'::jsonb)
                   FROM public.admin_users WHERE NOT (user_id = ANY(ids))), NULL);
