...
DECLARE
  ...
  v_req uuid; v_ord uuid; v_req2 uuid; v_ord2 uuid; v_reqC uuid; v_ordC uuid;
  ...
  v_jid uuid; v_created int; v_period text;
BEGIN
  ...
  BEGIN
    ...
    VALUES ('qa-s13-p6:'||v_sfx||':1','qa_s13', gen_random_uuid(), v_s1, v_m1,
            1000000, 1000000, 1000000, 'funded','qa');

    -- genuinely non-zero provider fee for the whole Part 6 run
    INSERT INTO public.provider_fee_schedules
      (provider, fee_bps, fee_fixed_gnf, passthrough_to_recipient, effective_from, note)
    VALUES ('orange_money', 0, 5000, true, clock_timestamp(), 'QA S13 P6 recipient-borne');
    ...
    r := r || public._qa_s13_ok('A1b the reservation freezes principal, fee and the exact expected transfer',
      ...
      format('principal=%s fee=%s expected=%s', v_o.requested_principal_gnf, v_o.provider_fee_gnf, v_o.expected_provider_transfer_gnf));
    ...
    SELECT COALESCE(sum(p.amount_gnf),0) INTO v_n FROM public.ledger_postings p
      JOIN public.ledger_accounts a ON a.code = p.account_code WHERE p.journal_id = v_jid AND a.code='A_PROVIDER_CLEARING';
    ...
    -- configured cadence policy (also flips the fee to platform-borne for the E series)
    ...
    VALUES (true, 10000, 300000, 'daily', false, clock_timestamp(), 'QA S13 P6 platform-borne daily');
    ...
    SELECT COALESCE(sum(p.amount_gnf),0) INTO v_n FROM public.ledger_postings p
     WHERE p.journal_id = (SELECT id FROM public.ledger_journals WHERE journal_key='payout-settle:'||v_ord::text)
       AND p.account_code = 'E_PROVIDER_FEE';
    ...
    r := r || public._qa_s13_ok('E2 platform-borne fee: 100000 principal, 5000 fee, 100000 transferred, 100000 debited',
      ...
      AND v_o.expected_provider_transfer_gnf = 100000 AND v_o.merchant_liability_debit_gnf = 100000, NULL);
    ...
    BEGIN
      PERFORM public.driver_cashout_mark_paid(gen_random_uuid(), 'OM-QA');
      v_err := 'no_error';
    EXCEPTION WHEN OTHERS THEN v_err := SQLERRM; END;
    r := r || public._qa_s13_ok('F4 the legacy driver cashout payment path stays hard-blocked',
      v_err LIKE 'STAGE_DISABLED%' OR v_err LIKE '%not authorized%' OR v_err LIKE '%NOT_AUTHORIZED%', v_err);
