...
BEGIN
  ...
  FOREACH f IN ARRAY ARRAY['_qa_s13_run4','_qa_s13_run7','_qa_node4_marche_r65','_qa_node4_marche_r7'] LOOP
    BEGIN
      ...
      IF arr IS NULL THEN
        tot := COALESCE((res->>'total')::int,0); fl := COALESCE((res->>'failed')::int,0);
      ELSE
