...
BEGIN
  ...
  -- ---- centralized bucket + confidence semantics ----
  ...
  BEGIN
    ...
    INSERT INTO public.merchant_stores(owner_user_id, slug, name, status, onboarding_status)
      VALUES (v_merch,'qa-n435-nc-'||substr(v_merch::text,1,8),'QA N435 Store NoCoords','active','approved')
    ...
    -- ---- happy path: 2 lines, known coords ----
    ...
    -- ---- category snapshot survives later listing edits ----
    PERFORM set_config('request.jwt.claims', public._as_user_claims(v_merch), true);
    ...
    -- ---- honest distance when coordinates are missing ----
    v_res := public.marche_order_commit(jsonb_build_object(
    ...
    -- ---- impossible interval: refused, never clamped ----
    v_res := public.marche_order_commit(jsonb_build_object(
    ...
    -- ---- cohort fixture: 10 delivery observations with known durations ----
    ...
    END LOOP;
    -- two pickup-mode rows that must never bleed into the delivery cohort
  ...
  DELETE FROM public.marche_order_items WHERE order_id IN
    (SELECT id FROM public.marche_orders WHERE buyer_user_id IN (v_buy, v_merch));
  DELETE FROM public.marche_orders WHERE buyer_user_id IN (v_buy, v_merch);
