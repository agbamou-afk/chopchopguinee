...
BEGIN
  ...
  res := res || jsonb_build_object('id',2,'name','operations reads finance facts but cannot act on them','pass',
  ...
  res := res || jsonb_build_object('id',36,'name','fixtures leave zero residue','pass',
    ...
    AND NOT EXISTS (SELECT 1 FROM public.wallets WHERE owner_user_id = ANY(ids))
