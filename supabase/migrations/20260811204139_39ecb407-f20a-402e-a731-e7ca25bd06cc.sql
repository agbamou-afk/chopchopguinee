...
BEGIN
  ...
  BEGIN
    ...
    BEGIN
      ...
      v_err := 'no_error';
    EXCEPTION WHEN OTHERS THEN v_err := SQLERRM; END;
    r := r || public._qa_s13_ok('F4 even a finance operator cannot use the legacy driver cashout payment path',
      v_err = 'STAGE_DISABLED:driver_cashout_enabled', v_err);
    PERFORM set_config('request.jwt.claims', public._as_user_claims(v_drv), true);
    BEGIN
      PERFORM public.driver_cashout_mark_paid(gen_random_uuid(), 'OM-QA-REF-2', 'qa');
      v_err := 'no_error';
    EXCEPTION WHEN OTHERS THEN v_err := SQLERRM; END;
    r := r || public._qa_s13_ok('F4b a driver cannot pay their own legacy cashout either',
      v_err = 'not_authorized', v_err);
