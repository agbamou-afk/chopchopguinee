-- ============================================================
-- NODE 5 · A8 — UNIFIED ACCOUNT / MODE SWITCHER (server truth)
-- Mode is PRESENTATION ONLY. It never grants identity, capability,
-- ownership, role, wallet, or any authorization.
-- ============================================================

-- 1) Canonical available-mode derivation (internal; service_role only)
CREATE OR REPLACE FUNCTION public.account_available_modes(p_user uuid)
RETURNS text[]
LANGUAGE sql
STABLE
SECURITY DEFINER
...
AS $$
  SELECT CASE public.professional_active_type(p_user)
           WHEN 'driver'   THEN ARRAY['client','driver']
           WHEN 'merchant' THEN ARRAY['client','merchant']
           ELSE ARRAY['client']
         END;
$$;
REVOKE ALL ON FUNCTION public.account_available_modes(uuid) FROM PUBLIC, anon, authenticated;
GRANT EXECUTE ON FUNCTION public.account_available_modes(uuid) TO service_role;

-- 2) Caller-scoped workspace context (the only client read surface)
CREATE OR REPLACE FUNCTION public.account_mode_context()
RETURNS jsonb
LANGUAGE plpgsql
STABLE
SECURITY DEFINER
...
BEGIN
  ...
  v_modes := public.account_available_modes(v_uid);
  SELECT app_mode INTO v_pref FROM public.user_preferences WHERE user_id = v_uid;

  RETURN jsonb_build_object(
    'professional_type',    COALESCE(v_row.professional_type, 'none'),
    'professional_identity_id', v_row.id,
    'claim_state',          v_row.claim_state,
    'available_modes',      to_jsonb(v_modes),
    'preferred_mode',       v_pref,
    'effective_mode',       CASE WHEN v_pref = ANY(v_modes) THEN v_pref ELSE 'client' END
  );
END $$;
REVOKE ALL ON FUNCTION public.account_mode_context() FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.account_mode_context() TO authenticated, service_role;

-- 3) Validated preference write. Never refuses hard — an unlawful
--    professional preference deterministically degrades to 'client'.
CREATE OR REPLACE FUNCTION public.account_mode_set(p_mode text)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path TO 'public'
...
BEGIN
  IF v_uid IS NULL THEN
    RAISE EXCEPTION 'AUTH_REQUIRED';
  END IF;
  IF p_mode IS NULL OR p_mode NOT IN ('client','driver','merchant') THEN
    RAISE EXCEPTION 'INVALID_MODE';
  END IF;

  v_modes := public.account_available_modes(v_uid);
  v_next  := CASE WHEN p_mode = ANY(v_modes) THEN p_mode ELSE 'client' END;

  INSERT INTO public.user_preferences(user_id, app_mode)
  VALUES (v_uid, v_next)
  ON CONFLICT (user_id) DO UPDATE SET app_mode = EXCLUDED.app_mode, updated_at = now();

  RETURN jsonb_build_object(
    'requested_mode', p_mode,
    'effective_mode', v_next,
    'available_modes', to_jsonb(v_modes),
    'refused', v_next <> p_mode
  );
END $$;
REVOKE ALL ON FUNCTION public.account_mode_set(text) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.account_mode_set(text) TO authenticated, service_role;

-- 4) Defense in depth: a direct table write can never persist an
--    unlawful professional preference (localStorage/API tampering).
CREATE OR REPLACE FUNCTION public._account_mode_preference_guard()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path TO 'public'
...
BEGIN
  IF NEW.app_mode IS NULL THEN
    RETURN NEW;
  END IF;
  IF NEW.app_mode NOT IN ('client','driver','merchant') THEN
    NEW.app_mode := 'client';
    RETURN NEW;
  END IF;
  IF NOT (NEW.app_mode = ANY(public.account_available_modes(NEW.user_id))) THEN
    NEW.app_mode := 'client';
  END IF;
  RETURN NEW;
END $$;

DROP TRIGGER IF EXISTS trg_account_mode_preference_guard ON public.user_preferences;
CREATE TRIGGER trg_account_mode_preference_guard
BEFORE INSERT OR UPDATE OF app_mode ON public.user_preferences
FOR EACH ROW EXECUTE FUNCTION public._account_mode_preference_guard();

-- ============================================================
-- A8 CERTIFICATION SUITE
-- ============================================================
...
DECLARE
  ...
  ctx jsonb; res jsonb; v_modes text[]; fn text; v_bad text[] := '{}';
...
BEGIN
  ...
  -- released driver: apply, then lawfully release the lane
  ...
  r := r || public._qa_s13_ok('N5A8.B11 pending driver still holds the driver mode (class, not status)',
        (SELECT status FROM public.driver_profiles WHERE user_id=u_d) <> 'approved'
  ...
  r := r || public._qa_s13_ok('N5A8.B12 submitted store still grants merchant mode (class, not approval)',
        (SELECT onboarding_status FROM public.merchant_stores WHERE id=s_m) = 'submitted'
  ...
  END;
  -- direct table tampering (the localStorage/API attack)
  ...
  r := r || public._qa_s13_ok('N5A8.D14 refused preference granted no role',
        NOT EXISTS (SELECT 1 FROM public.user_roles WHERE user_id=u_c), NULL);
  ...
  FOREACH fn IN ARRAY authority_fns LOOP
    IF to_regproc('public.'||fn) IS NOT NULL THEN
  ...
  r := r || public._qa_s13_ok('N5A8.G1 mode switching created no wallet',
        (SELECT count(*) FROM public.wallets WHERE user_id = ANY(ids)) = 0, NULL);
  r := r || public._qa_s13_ok('N5A8.G2 mode switching created no wallet transaction',
        (SELECT count(*) FROM public.wallet_transactions
          WHERE wallet_id IN (SELECT id FROM public.wallets WHERE user_id = ANY(ids))) = 0, NULL);
  ...
  r := r || public._qa_s13_ok('N5A8.G6 no payout order produced by mode switching',
        (SELECT count(*) FROM public.payout_orders WHERE beneficiary_user_id = ANY(ids)) = 0, NULL);
  ...
  r := r || public._qa_s13_ok('N5A8.I5 wallets unchanged', a_w = b_w, b_w||'->'||a_w);
