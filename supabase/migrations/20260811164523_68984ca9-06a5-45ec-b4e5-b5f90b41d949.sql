...
BEGIN
  ...
  BEGIN
    ...
    INSERT INTO public.merchant_stores(owner_user_id, name, slug, status)
    VALUES (v_own,'QA S13 Store','qa-s13-store-'||v_sfx,'approved') RETURNING id INTO v_store;
    INSERT INTO public.food_restaurants(slug, name, merchant_store_id, owner_user_id, status)
    VALUES ('qa-s13-resto-'||v_sfx,'QA S13 Resto', v_store, v_own, 'approved')
