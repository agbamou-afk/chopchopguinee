...
DECLARE
  ...
  v_n int; v_n2 int; v_row jsonb; v_case uuid; v_case2 uuid;
  v_rq uuid; v_rq2 uuid; v_rq3 uuid; v_rq4 uuid; v_rq5 uuid;
...
BEGIN
  ...
  -- H. no direct CRUD for ordinary admin clients
  ...
  r := r || public._qa_s13_ok('P0.16 authenticated still has no direct write on food_orders',
        NOT has_table_privilege('authenticated','public.food_orders','DELETE'), NULL);
  ...
  BEGIN
    ...
    -- Age order A so a real stuck condition exists.
    PERFORM set_config('chopchop.cash_engine','1',true);
    UPDATE public.food_orders SET updated_at = now() - interval '95 minutes',
                                  created_at = now() - interval '95 minutes'
     WHERE id = v_oA;
    ...
    r := r || public._qa_s13_ok('P2.4 the aged order surfaces in the attention queue',
          (v_row->>'order_id')::uuid = v_oA, v_row::text);
    ...
    -- E5: prior events cannot be rewritten
    ...
    v_res := public.repas_ops_command(v_oC,'reopen',gen_random_uuid(),
              'quality_issue','QA R10 reouverture');
    v_case2 := (v_res->>'case_id')::uuid;
    ...
    r := r || public._qa_s13_ok('P8.6 the cancellation moved value exactly once',
          v_jr2 >= v_jr, v_jr::text||'/'||v_jr2::text);
    ...
    r := r || public._qa_s13_ok('P9.4 a cancelled order raises no fabricated attention flag',
          (v_q->'rows'->0->>'attention_flags') IN ('[]','["dispute_open"]'),
...
CREATE OR REPLACE FUNCTION public._qa_node3_repas_r10_operations()
RETURNS jsonb LANGUAGE plpgsql SECURITY DEFINER SET search_path TO 'public' AS $function$
DECLARE v_out jsonb; BEGIN
  PERFORM set_config('app.repas_fixture_verified','1', true);
  v_out := public._qa_node3_repas_r10_operations_fxcore();
  PERFORM set_config('app.repas_fixture_verified','', true);
  RETURN v_out; END; $function$;

REVOKE ALL ON FUNCTION public._qa_node3_repas_r10_operations() FROM PUBLIC, anon, authenticated;
REVOKE ALL ON FUNCTION public._qa_node3_repas_r10_operations_fxcore() FROM PUBLIC, anon, authenticated;