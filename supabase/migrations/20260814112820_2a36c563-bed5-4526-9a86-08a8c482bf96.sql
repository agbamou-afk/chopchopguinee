...
BEGIN
  ...
  BEGIN
    ...
    BEGIN PERFORM public.repas_custody_confirm_handoff(v_m1,'qa/p.jpg','000000'); v_err := 'NO_ERROR';
    ...
    BEGIN PERFORM public.repas_custody_confirm_handoff(v_m1,'qa/p.jpg','000000'); v_err := 'NO_ERROR';
    EXCEPTION WHEN OTHERS THEN v_err := SQLERRM; END;
    r := r || public._qa_s13_ok('A1.3 handoff refused while the order is not ready',
          v_err LIKE '%ORDER_NOT_READY%', v_err);
    ...
    BEGIN PERFORM public.repas_custody_confirm_handoff(v_m1,'qa/p.jpg','111111'); v_err := 'NO_ERROR';
    EXCEPTION WHEN OTHERS THEN v_err := SQLERRM; END;
    r := r || public._qa_s13_ok('A1.21 a wrong code refuses the handoff',
          v_err LIKE '%CUSTODY_CODE_INVALID%' OR v_err LIKE '%CUSTODY_CODE_ALREADY_USED%', v_err);
    ...
    SELECT count(*) INTO v_n FROM public.mission_events
     WHERE mission_id=v_m1 AND kind='repas_custody_restaurant_to_courier';
    ...
    r := r || public._qa_s13_ok('A2.6 bare mission_confirm_dropoff cannot bypass custody',
          v_err LIKE '%REPAS_CUSTODY_REQUIRED%', v_err);
    BEGIN PERFORM public.mission_confirm_dropoff_with_proof(v_m1,'qa/d.jpg',NULL); v_err := 'NO_ERROR';
    EXCEPTION WHEN OTHERS THEN v_err := SQLERRM; END;
    ...
    BEGIN PERFORM public.repas_custody_confirm_delivery(v_m1,'qa/d.jpg','999999'); v_err := 'NO_ERROR';
    EXCEPTION WHEN OTHERS THEN v_err := SQLERRM; END;
    r := r || public._qa_s13_ok('A2.10 a wrong customer code refuses the delivery',
          v_err LIKE '%CUSTODY_CODE_INVALID%', v_err);
    SELECT attempts INTO v_att FROM public.repas_custody_credentials
     WHERE order_id=v_o1 AND kind='customer_delivery';
    r := r || public._qa_s13_ok('A2.11 the failed attempt is counted server-side', v_att = 1, v_att::text);
    ...
    SELECT count(*) INTO v_n FROM public.ledger_journals
      WHERE source_module='repas' AND source_id=v_o1 AND journal_key LIKE 'cph-capture:%delivery';
    r := r || public._qa_s13_ok('A2.23 exactly one delivery capture journal', v_n = 1, v_n::text);

    PERFORM set_config('request.jwt.claims', public._as_user_claims(v_cust), true);
    v_view := public.repas_custody_code_view(v_o1,'customer_delivery');
    r := r || public._qa_s13_ok('A2.24 a terminal order exposes no live credential',
    ...
    r := r || public._qa_s13_ok('C1.1 each order gets a distinct credential row',
    ...
    FOR v_i IN 1..5 LOOP
      ...
      EXCEPTION WHEN OTHERS THEN v_err := SQLERRM; END;
    END LOOP;
    SELECT attempts, (locked_at IS NOT NULL) INTO v_att, v_state
      FROM public.repas_custody_credentials WHERE order_id=v_o3 AND kind='restaurant_handoff';
    r := r || public._qa_s13_ok('C1.2 five wrong codes are counted', v_att = 5, v_att::text);
    r := r || public._qa_s13_ok('C1.3 the credential is locked after the cap', v_state = 'true', v_state);
    ...
    r := r || public._qa_s13_ok('C1.6 a locked credential is no longer displayable',
          (v_view->>'locked')::boolean IS TRUE AND (v_view->>'code') IS NULL, v_view::text);
    ...
    -- ================= A2-neg. DELIVERY WITHOUT PICKUP CUSTODY =================
    PERFORM set_config('request.jwt.claims', public._as_user_claims(v_cust), true);
    v_res := public.repas_order_create(v_resto,
        jsonb_build_array(jsonb_build_object('menu_item_id',v_item,'qty',1)),
    ...
    BEGIN PERFORM public.repas_custody_confirm_pickup_collection(v_o5,'000000'); v_err := 'NO_ERROR';
    EXCEPTION WHEN OTHERS THEN v_err := SQLERRM; END;
    r := r || public._qa_s13_ok('B1.7 a wrong Retrait code is refused',
          v_err LIKE '%CUSTODY_CODE_INVALID%', v_err);
    ...
    r := r || public._qa_s13_ok('B1.11 a valid customer code completes the Retrait',
          (v_res->>'ok')::boolean AND (v_res->>'state')='completed', v_res::text);
    SELECT * INTO v_cp FROM public.chop_pay_order_runtime
    ...
    r := r || public._qa_s13_ok('B1.18 replaying Retrait returns canonical truth and moves zero value',
          (v_res->>'idempotent')::boolean IS TRUE AND v_mm1 = v_bal, v_res::text);
    SELECT count(*) INTO v_n FROM public.repas_custody_events
    ...
    SELECT count(*) INTO v_n FROM public.ledger_journals
      WHERE source_module='repas' AND source_id=v_o5 AND journal_key LIKE '%platform_fee%';
    r := r || public._qa_s13_ok('B1.20 exactly one Retrait platform fee journal', v_n = 1, v_n::text);
    ...
    r := r || public._qa_s13_ok('B1.21 no driver-side hold touched the Retrait order', v_n = 0, v_n::text);
    ...
    r := r || public._qa_s13_ok('D1.1 custody events cannot be rewritten',
          v_err LIKE '%CUSTODY_EVENTS_APPEND_ONLY%', v_err);
    ...
    r := r || public._qa_s13_ok('E1.4 every journal created here is zero-sum', v_unbalanced = 0, v_unbalanced::text);
