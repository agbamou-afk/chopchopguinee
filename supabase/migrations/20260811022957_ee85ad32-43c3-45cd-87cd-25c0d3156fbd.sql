...
CREATE OR REPLACE FUNCTION public._qa_s8_run2()
RETURNS jsonb LANGUAGE plpgsql SET search_path TO 'public'
AS $fn$
DECLARE
  r jsonb := '[]'::jsonb;
  ride_snap jsonb; repas_snap jsonb; envoyer_snap jsonb; marche_snap jsonb; new_snap jsonb;
  c jsonb; c2 jsonb;
  cust uuid := gen_random_uuid();
  wid uuid; master_before bigint; master_after bigint;
  d jsonb; d2 jsonb; debt_id uuid; src_id uuid; rep jsonb; rep2 jsonb;
...
BEGIN
  ...
  r := r || public._qa_s8_ok('B3 driver-caused -> 0',
        (public._cancellation_compute(ride_snap,'after_dispatch',100000,0,0,'driver')->>'fee_gnf')::bigint = 0, '{}');
  r := r || public._qa_s8_ok('B4 platform-caused -> 0',
        (public._cancellation_compute(ride_snap,'after_dispatch',100000,0,0,'platform')->>'fee_gnf')::bigint = 0, '{}');
  r := r || public._qa_s8_ok('B5 provider-caused -> 0',
        (public._cancellation_compute(ride_snap,'after_dispatch',100000,0,0,'provider')->>'fee_gnf')::bigint = 0, '{}');
  ...
  r := r || public._qa_s8_ok('C3 merchant rejection -> 0 fee',
        (public._cancellation_compute(repas_snap,'after_dispatch',0,150000,25000,'merchant')->>'fee_gnf')::bigint = 0, '{}');
  ...
  r := r || public._qa_s8_ok('D3 platform transaction fee excluded from basis',
        (public._cancellation_compute(repas_snap,'before_dispatch',0,150000,25000,'customer')->>'basis_gnf')::bigint = 175000,
        jsonb_build_object('note','merchandise + delivery only'));
  ...
  BEGIN
    ...
    r := r || public._qa_s8_ok('G5 exactly one canonical calculator exists',
  ...
  -- ================= F. DEBT / RESTRICTION / REPAYMENT + H. LEDGER =================
  BEGIN
    ...
    src_id := gen_random_uuid();
    d := public._customer_cancellation_debt_create_internal(
           'ride', src_id, cust, 'ride', 'after_dispatch',
    ...
    SELECT count(*) INTO n FROM public.customer_cancellation_debts WHERE source_id = src_id;
    r := r || public._qa_s8_ok('F2 exactly one debt row per source', n = 1, jsonb_build_object('rows', n));
    ...
    SELECT count(*) INTO n FROM public.customer_cancellation_debts WHERE source_id = src_id;
    r := r || public._qa_s8_ok('F3b replay created no second debt', n = 1, jsonb_build_object('rows', n));
    ...
    r := r || public._qa_s8_ok('F5 outstanding debt restricts new cash exposure',
          public._customer_cash_restricted(cust) = true, '{}');
    ...
    r := r || public._qa_s8_ok('F9 outstanding exact and never negative', z = 4000, jsonb_build_object('outstanding', z));
    ...
    r := r || public._qa_s8_ok('F11 full repayment restores cash eligibility automatically',
          public._customer_cash_restricted(cust) = false, '{}');
    ...
    r := r || public._qa_s8_ok('F13 customer wallet debited exactly 10000', z = 0, jsonb_build_object('balance', z));
    SELECT paid_gnf INTO z FROM public.customer_cancellation_debts WHERE id = debt_id;
    r := r || public._qa_s8_ok('F13b collected total equals the fee, no overcollection', z = 10000,
          jsonb_build_object('paid', z));
    ...
    r := r || public._qa_s8_ok('F14 waiver records waived_gnf, never paid_gnf',
          (SELECT waived_gnf = 5000 AND paid_gnf = 0
             FROM public.customer_cancellation_debts WHERE id = (d2->>'debt_id')::uuid), rep2);
    r := r || public._qa_s8_ok('F15 waiver is not payment (distinct ledger action)',
          EXISTS (SELECT 1 FROM public.ledger_journals
                   WHERE action = 'cancellation_fee_waived'
                     AND source_id = (SELECT source_id FROM public.customer_cancellation_debts
                                       WHERE id = (d2->>'debt_id')::uuid)), '{}');
    ...
    END;
    SELECT count(*) INTO n FROM public.customer_cancellation_debts
     WHERE customer_user_id = cust AND mission_type = 'repas';
    r := r || public._qa_s8_ok('C5 preparation lock caused zero finance mutation', n = 0,
          jsonb_build_object('repas_debts', n));

    -- H. LEDGER
    SELECT count(*) INTO n FROM public.ledger_journals
     WHERE action IN ('cancellation_fee_charged','cancellation_fee_collected','cancellation_fee_waived')
    ...
    SELECT count(*) INTO n FROM public.ledger_journals
     WHERE action = 'cancellation_fee_charged' AND source_id = src_id;
    r := r || public._qa_s8_ok('H3 no duplicate platform cancellation revenue', n = 1,
          jsonb_build_object('charge_journals', n));
    SELECT count(*) INTO n FROM public.ledger_journals
     WHERE action = 'cancellation_fee_collected' AND source_id = src_id;
    r := r || public._qa_s8_ok('H4 three partial collections, each posted once', n = 3,
          jsonb_build_object('collect_journals', n));
    SELECT count(*) INTO n FROM public.ledger_journals
     WHERE created_at > now() - interval '5 minutes' AND source_id IS NULL;
    r := r || public._qa_s8_ok('H5 every cancellation journal is source-linked', n = 0,
          jsonb_build_object('unlinked', n));
  ...
  -- ================= I. PRIVILEGES =================
  SELECT COALESCE(array_to_string(proacl,','),'DEFAULT') INTO acl FROM pg_proc p
    JOIN pg_namespace nn ON nn.oid=p.pronamespace WHERE nn.nspname='public' AND p.proname='_cancellation_compute';
  r := r || public._qa_s8_ok('I1 calculator closed to anon and authenticated',
        acl NOT LIKE '%anon=%' AND acl NOT LIKE '%authenticated=%', jsonb_build_object('acl', acl));
  SELECT COALESCE(array_to_string(proacl,','),'DEFAULT') INTO acl FROM pg_proc p
    JOIN pg_namespace nn ON nn.oid=p.pronamespace WHERE nn.nspname='public' AND p.proname='cancellation_quote';
  ...
  SELECT COALESCE(array_to_string(proacl,','),'DEFAULT') INTO acl FROM pg_proc p
    JOIN pg_namespace nn ON nn.oid=p.pronamespace WHERE nn.nspname='public' AND p.proname='customer_cancellation_debt_repay';
  r := r || public._qa_s8_ok('I3 repayment denied to anon, self-only for authenticated',
        acl NOT LIKE '%anon=%' AND acl LIKE '%authenticated=%', jsonb_build_object('acl', acl));
  SELECT COALESCE(array_to_string(proacl,','),'DEFAULT') INTO acl FROM pg_proc p
    JOIN pg_namespace nn ON nn.oid=p.pronamespace WHERE nn.nspname='public' AND p.proname='customer_cancellation_debt_waive';
  r := r || public._qa_s8_ok('I4 waiver never exposed to anon or authenticated',
        acl NOT LIKE '%anon=%' AND acl NOT LIKE '%authenticated=%', jsonb_build_object('acl', acl));
  SELECT COALESCE(array_to_string(proacl,','),'DEFAULT') INTO acl FROM pg_proc p
    JOIN pg_namespace nn ON nn.oid=p.pronamespace WHERE nn.nspname='public' AND p.proname='customer_cancellation_debt_collect';
  r := r || public._qa_s8_ok('I5 collection primitive closed',
        acl NOT LIKE '%anon=%' AND acl NOT LIKE '%authenticated=%', jsonb_build_object('acl', acl));
  SELECT COALESCE(array_to_string(proacl,','),'DEFAULT') INTO acl FROM pg_proc p
    JOIN pg_namespace nn ON nn.oid=p.pronamespace WHERE nn.nspname='public' AND p.proname='_customer_cancellation_debt_settle_internal';
  r := r || public._qa_s8_ok('I6 settle internal is service-role only',
        acl NOT LIKE '%anon=%' AND acl NOT LIKE '%authenticated=%', jsonb_build_object('acl', acl));
  SELECT COALESCE(array_to_string(proacl,','),'DEFAULT') INTO acl FROM pg_proc p
    JOIN pg_namespace nn ON nn.oid=p.pronamespace WHERE nn.nspname='public' AND p.proname='_package_cancel_release_internal';
  r := r || public._qa_s8_ok('I7 package release internal is service-role only',
        acl NOT LIKE '%anon=%' AND acl NOT LIKE '%authenticated=%', jsonb_build_object('acl', acl));
  SELECT COALESCE(array_to_string(proacl,','),'DEFAULT') INTO acl FROM pg_proc p
    JOIN pg_namespace nn ON nn.oid=p.pronamespace WHERE nn.nspname='public' AND p.proname='_customer_cash_restricted';
  ...
  SELECT count(*) INTO n FROM pg_proc p JOIN pg_namespace nn ON nn.oid=p.pronamespace
   WHERE nn.nspname='public'
     AND p.proname IN ('driver_funding_allocate','_promo_consume','driver_promo_balance',
                       '_ledger_post','_ledger_reverse','claims_reserve_allocate')
     AND COALESCE(array_to_string(p.proacl,','),'DEFAULT') LIKE '%authenticated=%';
  r := r || public._qa_s8_ok('I9 no Slice 3-7 privilege regression on raw primitives', n = 0,
        jsonb_build_object('leaked', n));
  SELECT count(*) INTO n FROM pg_proc p JOIN pg_namespace nn ON nn.oid=p.pronamespace
   WHERE nn.nspname='public' AND p.proname LIKE '%cancellation%'
     AND pg_get_functiondef(p.oid) LIKE '%auth.uid() IS NULL%';
  r := r || public._qa_s8_ok('I10 no auth.uid() IS NULL trusted shortcut', n = 0, jsonb_build_object('found', n));
  ...
  r := r || public._qa_s8_ok('A1 all cancellation paths call the one calculator', n >= 5,
        jsonb_build_object('paths_wired', n));
  SELECT count(*) INTO n FROM pg_proc p JOIN pg_namespace nn ON nn.oid=p.pronamespace
  ...
  r := r || public._qa_s8_ok('A2 hard-coded envoyer 10 percent formula removed', n = 0,
  ...
  r := r || public._qa_s8_ok('J1 master wallet unchanged by harness', master_before = master_after,
  ...
  r := r || public._qa_s8_ok('J2 zero debt fixture residue', n = 0, jsonb_build_object('rows', n));
  SELECT count(*) INTO n FROM public.wallets WHERE owner_user_id = cust;
  r := r || public._qa_s8_ok('J3 zero wallet fixture residue', n = 0, jsonb_build_object('rows', n));
  SELECT count(*) INTO n FROM public.finance_policies WHERE cancel_before_dispatch_bps = 2500;
  r := r || public._qa_s8_ok('J4 zero policy fixture residue', n = 0, jsonb_build_object('rows', n));
  SELECT count(*) INTO n FROM public.ledger_journals WHERE created_at > now() - interval '5 minutes';
  r := r || public._qa_s8_ok('J5 zero ledger fixture residue', n = 0, jsonb_build_object('rows', n));
  SELECT count(*) INTO n FROM public.feature_flags
   WHERE key IN ('chop_pay_enabled','chop_pay_checkout_enabled','cash_order_funding_enabled',
     ...
     AND enabled = true;
  r := r || public._qa_s8_ok('J6 no finance rail activated', n = 0, jsonb_build_object('enabled', n));
  r := r || public._qa_s8_ok('J7 om_topup_enabled preserved ON',
        (SELECT enabled FROM public.feature_flags WHERE key='om_topup_enabled') = true, '{}');

  RETURN r;
END; $fn$;

DELETE FROM public._qa_s8_results;
INSERT INTO public._qa_s8_results (section, payload)
SELECT 'slice8', public._qa_s8_run2();

DROP FUNCTION public._qa_s8_run2();
