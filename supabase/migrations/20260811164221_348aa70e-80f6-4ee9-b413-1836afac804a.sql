...
BEGIN
  ...
  BEGIN
    ...
    PERFORM public._qa_s13_wallet(v_d2,'driver',1000,0);
    ...
    -- ---------- A9: starter credit entitlement (internal approval service) ----------
    PERFORM public._qa_s13_flag('driver_starter_credit_enabled', true);
    PERFORM set_config('request.jwt.claims', '', true);
    UPDATE public.profiles SET phone = '+224622000111' WHERE user_id = v_d1;
    ...
    -- same human identity, different raw formatting
    ...
    r := r || public._qa_s13_ok('A9.6 no second grant issued to the duplicate identity', v_n = 0, v_n::text);
