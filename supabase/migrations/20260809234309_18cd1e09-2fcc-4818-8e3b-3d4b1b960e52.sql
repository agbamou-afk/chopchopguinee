...
BEGIN
  ...
  UPDATE public.wallets SET balance_gnf = 0, held_gnf = 0, status='active'
    WHERE owner_user_id IN (u_d,u_c);
  ...
  IF v_dw IS NULL OR v_cw IS NULL THEN RAISE EXCEPTION 'QA_S3D_RESULT %', jsonb_build_object('fatal','wallet_setup'); END IF;
