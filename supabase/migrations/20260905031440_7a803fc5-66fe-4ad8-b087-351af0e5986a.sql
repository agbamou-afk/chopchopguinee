...
BEGIN
  ...
  PERFORM public.admin_staff_record_auth_as(rid, t1);
  PERFORM public.admin_staff_record_auth_as(rid, gen_random_uuid());
  ...
  EXCEPTION WHEN OTHERS THEN ok := SQLERRM ILIKE '%mismatch%'; END;
  IF ok THEN
    ...
    BEGIN
      PERFORM public.admin_staff_lifecycle_begin_as(godA,'DEACTIVATE','qa-g3-replay-'||replace(t2::text,'-',''),
              t2::text,NULL,false,ap,t2,'qa');
    EXCEPTION WHEN OTHERS THEN ok := SQLERRM ILIKE '%consumed%' OR SQLERRM ILIKE '%approval%'; END;
  END IF;
  ...
  res := res || jsonb_build_object('id',23,'name','pre-existing real Operations Admin accounts unchanged',
    'pass', fp_before IS NOT NULL AND fp_before = fp_after);

  PERFORM public._qa_g3_purge_fixtures(ids);
