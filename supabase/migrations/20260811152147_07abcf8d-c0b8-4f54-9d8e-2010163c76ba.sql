...
DECLARE
  ...
  v_allon boolean; vf1 uuid; vf2 uuid; vp1 uuid;
BEGIN
  BEGIN
    ...
    r := r || jsonb_build_object('name','A22 unconfigured scheduler never queues or double-queues',
    ...
    SELECT count(*) INTO v_cnt FROM public.ledger_postings p
     ...
     WHERE j.journal_key = 'payout-settle:' || vf1::text AND p.account_code = 'E_PROVIDER_FEE';
    ...
    SELECT count(*), COALESCE(SUM(p.amount_gnf),0) INTO v_cnt, v_sum
      ...
      JOIN public.ledger_journals j ON j.id = p.journal_id
     WHERE j.journal_key = 'payout-settle:' || vp1::text AND p.account_code = 'E_PROVIDER_FEE';
