...
DECLARE
  ...
  v_r text; v_j jsonb; v_a bigint; v_b bigint; v_c bigint; v_d bigint; v_ok boolean;
...
BEGIN
  ...
  BEGIN
    ...
    EXCEPTION WHEN OTHERS THEN v_r := 'ERR: '||SQLERRM; END;
    v_out := v_out || jsonb_build_object('id','I3','case','suspended driver bonus request grants nothing','obs',v_r,
    ...
    SELECT NOT bool_or(has_function_privilege('authenticated', p.oid, 'execute')) INTO v_ok
      FROM pg_proc p JOIN pg_namespace n ON n.oid = p.pronamespace
     WHERE n.nspname = 'public'
       AND p.proname IN ('driver_funding_allocate','_promo_consume','_promo_restore','driver_promo_balance');
    ...
    SELECT count(*) = 0 INTO v_ok FROM public.mission_financial_holds
     WHERE COALESCE(captured_promo_gnf,0) + COALESCE(captured_unrestricted_gnf,0) > COALESCE(captured_gnf,0);
