...
BEGIN
  ...
  BEGIN
    ...
    r := r || public._qa_s13_ok('E1.1 the customer was debited exactly the settled order totals',
          (v_c0 - v_c1) = v_sum_total,
          'debited='||(v_c0 - v_c1)::text||' expected='||v_sum_total::text||' held='||v_held::text);
    r := r || public._qa_s13_ok('E1.1b the merchant received exactly the settled principal',
          (v_mm1 - v_mm0) = v_sum_m, (v_mm1 - v_mm0)::text||'/'||v_sum_m::text);
    r := r || public._qa_s13_ok('E1.1c the courier received exactly the settled earnings',
          (v_d1 - v_d0) = v_sum_d, (v_d1 - v_d0)::text||'/'||v_sum_d::text);
    r := r || public._qa_s13_ok('E1.1d the platform received exactly the settled fees',
          (v_x1 - v_x0) = v_sum_p, (v_x1 - v_x0)::text||'/'||v_sum_p::text);
    r := r || public._qa_s13_ok('E1.1e nothing was created or destroyed in between',
