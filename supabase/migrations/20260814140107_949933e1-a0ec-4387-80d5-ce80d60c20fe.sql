...
BEGIN
  ...
  r := r || public._qa_s13_ok('P0.20 service_role keeps full access for server code',
        has_column_privilege('service_role','public.missions','estimated_earning_gnf','SELECT')
    AND has_column_privilege('service_role','public.food_orders','courier_payout_gnf','SELECT'), NULL);

  SELECT pg_get_functiondef(p.oid) INTO v_def FROM pg_proc p JOIN pg_namespace n ON n.oid=p.pronamespace
   WHERE n.nspname='public' AND p.proname='repas_order_tracking';
  r := r || public._qa_s13_ok('P0.21 tracking fails closed on anonymous callers',
        v_def LIKE '%NOT_AUTHENTICATED%' AND v_def LIKE '%NOT_AUTHORIZED%', NULL);
  r := r || public._qa_s13_ok('P0.22 tracking derives merchant actions server-side',
        v_def LIKE '%allowed_actions%', NULL);
  SELECT pg_get_functiondef(p.oid) INTO v_def FROM pg_proc p JOIN pg_namespace n ON n.oid=p.pronamespace
   WHERE n.nspname='public' AND p.proname='repas_order_receipt';
  r := r || public._qa_s13_ok('P0.23 receipt fails closed on anonymous callers',
        v_def LIKE '%NOT_AUTHENTICATED%' AND v_def LIKE '%NOT_AUTHORIZED%', NULL);
  r := r || public._qa_s13_ok('P0.24 receipt reconciles totals server-side',
        v_def LIKE '%totals_reconcile%', NULL);
  r := r || public._qa_s13_ok('P0.25 receipt lines use the frozen name snapshot',
  ...
  BEGIN
    ...
    BEGIN PERFORM public.repas_order_tracking(v_o1); v_err := 'NO_ERROR';
    EXCEPTION WHEN OTHERS THEN v_err := SQLERRM; END;
    r := r || public._qa_s13_ok('P1.3 a stranger cannot track another customer order',
          v_err LIKE '%NOT_AUTHORIZED%', v_err);
    BEGIN PERFORM public.repas_order_receipt(v_o1); v_err := 'NO_ERROR';
    EXCEPTION WHEN OTHERS THEN v_err := SQLERRM; END;
    r := r || public._qa_s13_ok('P1.4 a stranger cannot read another customer receipt',
          v_err LIKE '%NOT_AUTHORIZED%', v_err);
    PERFORM set_config('request.jwt.claims', public._as_user_claims(v_cust), true);
    ...
    r := r || public._qa_s13_ok('P2.9 customer never receives another party contact block',
    ...
    v_res := public.repas_order_create(v_resto,
        jsonb_build_array(jsonb_build_object('menu_item_id',v_item,'qty',1)),
        'delivery','choppay', gen_random_uuid(), 'QA R7 Adresse', 9.5395, -13.6760);
...
DROP FUNCTION IF EXISTS public._qa_node3_repas_r7_fixture_geo(uuid);
