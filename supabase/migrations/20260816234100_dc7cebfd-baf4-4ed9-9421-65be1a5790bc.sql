...
DECLARE t int; f int;
BEGIN
  CREATE TEMP TABLE _r5c AS SELECT * FROM public._qa_node3_repas_r5();
  SELECT count(*) INTO t FROM _r5c;
  EXECUTE 'SELECT count(*) FROM _r5c WHERE NOT (' ||
    (SELECT string_agg(quote_ident(column_name),' ') FROM information_schema.columns
      WHERE table_name='_r5c' AND data_type='boolean' LIMIT 1) || ')' INTO f;
...
SELECT jsonb_build_object(
  ...
  'flags_now', (SELECT jsonb_object_agg(key, enabled) FROM public.feature_flags WHERE key like 'chop_pay%'),
