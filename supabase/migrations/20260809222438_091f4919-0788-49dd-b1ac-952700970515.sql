CREATE OR REPLACE FUNCTION public._as_user_claims(p_user uuid)
RETURNS text LANGUAGE sql IMMUTABLE SET search_path = public AS $$
  SELECT jsonb_build_object('sub', p_user::text, 'role', 'authenticated')::text
$$;
REVOKE ALL ON FUNCTION public._as_user_claims(uuid) FROM PUBLIC, anon, authenticated;
...
BEGIN
  ...
  IF v_pay = 'chop_pay' AND v_ride.hold_tx_id IS NOT NULL THEN
    ...
    -- Capture runs as the paying customer (hold owner).
    ...
    IF v_ride.driver_id IS NOT NULL AND v_commission > 0 AND v_payment.id IS NOT NULL THEN
      -- Commission transfer runs as the driver (source wallet owner).
      PERFORM set_config('request.jwt.claims', public._as_user_claims(v_ride.driver_id), true);
      BEGIN
