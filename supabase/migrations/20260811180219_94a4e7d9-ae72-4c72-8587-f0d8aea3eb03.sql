-- ============================================================
-- Slice 13 · Part 4 closeout
-- (1) product fix: canonical courier cancellation before custody
-- (2) QA: storage/RLS probe helper
-- (3) QA: extended Part 4 harness
-- ============================================================

-- ---------- (1) Courier cancellation before custody ----------
CREATE OR REPLACE FUNCTION public.package_courier_cancel(p_package_id uuid, p_reason text DEFAULT NULL::text)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
...
DECLARE
  ...
  v_m public.missions; v_s public.package_delivery_secrets; v_rel jsonb;
BEGIN
  ...
  IF NOT (COALESCE(v_m.courier_id, v_rt.driver_user_id) = v_uid) THEN
    RAISE EXCEPTION 'forbidden' USING ERRCODE='42501',
      DETAIL = 'Only the assigned courier can release a shipment';
  END IF;

  IF v_pkg.cancelled_at IS NOT NULL OR v_rt.state = 'cancelled' THEN
    RETURN jsonb_build_object('idempotent', true, 'status','cancelled',
      'cancellation_fee_gnf', COALESCE(v_pkg.cancellation_fee_gnf,0), 'responsible_party','driver');
  END IF;

  IF COALESCE(v_rt.claim_state,'none') <> 'none' THEN
    RAISE EXCEPTION 'SETTLEMENT_FROZEN_BY_CLAIM' USING ERRCODE='22023';
  END IF;

  IF v_s.pickup_verified_at IS NOT NULL
     OR v_pkg.package_status IN ('in_transit','delivered')
     OR COALESCE(v_rt.state,'') IN ('picked_up','completed') THEN
    RAISE EXCEPTION 'CUSTODY_ESTABLISHED_CLAIM_REQUIRED' USING ERRCODE='22023',
      DETAIL = 'A courier holding the parcel must open a support claim, not cancel';
  END IF;

  -- Canonical cancellation primitive. responsible_party = driver, so the
  -- customer is never charged a cancellation fee and declared value is never
  -- part of any basis.
  v_rel := public._package_cancel_release_internal(
             p_package_id, COALESCE(p_reason,'courier_cancelled_before_custody'), v_uid, 'driver');

  IF v_m.id IS NOT NULL THEN
    UPDATE public.missions
       SET state = 'failed'::public.mission_state,
           issue_reason = left(COALESCE(p_reason,'courier_cancelled_before_custody'), 300)
     WHERE id = v_m.id;
  END IF;

  UPDATE public.package_deliveries
     SET package_status = 'cancelled', cancelled_at = now(),
         cancellation_reason = left(COALESCE(p_reason,'courier_cancelled_before_custody'), 300),
         cancellation_fee_gnf = 0
   WHERE id = p_package_id;

  INSERT INTO public.audit_logs (actor_user_id, module, action, target_type, target_id, after, note)
  VALUES (v_uid,'package','package.courier_cancelled','package_delivery', p_package_id::text,
          jsonb_build_object('release', v_rel, 'responsible_party','driver',
                             'cancellation_fee_gnf', 0), p_reason);

  BEGIN
    PERFORM public._package_notify(
      v_pkg.sender_user_id, 'package_courier_cancelled',
      jsonb_build_object('package_id', v_pkg.id, 'reference', v_pkg.reference,
                         'sandbox', v_pkg.is_sandbox), 'high');
  EXCEPTION WHEN OTHERS THEN NULL; END;

  RETURN jsonb_build_object('idempotent', false, 'status','cancelled',
    'responsible_party','driver', 'cancellation_fee_gnf', 0, 'release', v_rel);
END; $function$;

REVOKE ALL ON FUNCTION public.package_courier_cancel(uuid, text) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.package_courier_cancel(uuid, text) TO authenticated, service_role;

-- ---------- (2) QA storage / RLS probe ----------
CREATE OR REPLACE FUNCTION public._qa_s13_rls_probe(p_role text, p_uid uuid, p_op text,
...
CREATE OR REPLACE FUNCTION public._qa_s13_driver(p_uid uuid, p_tag text, p_bal bigint)
RETURNS void
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path TO 'public'
AS $function$
BEGIN
  PERFORM public._qa_s13_user(p_uid, p_tag);
  INSERT INTO public.driver_profiles(user_id,status,vehicle_type,id_doc_url,vehicle_photo_url)
  VALUES (p_uid,'approved','moto','x','y')
  ON CONFLICT (user_id) DO UPDATE SET status='approved';
  UPDATE public.driver_profiles
     SET capabilities = ARRAY['rides_moto','repas_delivery','marche_delivery','package_delivery']
   WHERE user_id = p_uid;
  PERFORM public._qa_s13_wallet(p_uid,'driver',p_bal,0);
END $function$;

REVOKE ALL ON FUNCTION public._qa_s13_driver(uuid, text, bigint) FROM PUBLIC, anon, authenticated;
GRANT EXECUTE ON FUNCTION public._qa_s13_driver(uuid, text, bigint) TO service_role;

-- ---------- (3) Extended Part 4 harness ----------
...
BEGIN
  ...
  BEGIN
    ...
    -- non-courier cannot use the courier release path
    PERFORM set_config('request.jwt.claims', public._as_user_claims(v_cust), true);
    ...
    -- custody established -> courier cancellation must be refused
    PERFORM set_config('request.jwt.claims', public._as_user_claims(v_cust), true);
    ...
    -- ===== CLOSEOUT 4: compensation is capped by the minimum of the three limits =====
    -- Fixture A: accepted declared value is the binding constraint.
    PERFORM set_config('request.jwt.claims', public._as_user_claims(v_cust), true);
    ...
    -- Fixture B: the active claims limit is the binding constraint.
    ...
    -- Fixture C: the documented actual value is the binding constraint.
    PERFORM set_config('request.jwt.claims', public._as_user_claims(v_cust), true);
