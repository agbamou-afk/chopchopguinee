...
BEGIN
  ...
  SELECT count(*) INTO v_n FROM cron.job
    WHERE jobname='chopchop-ride-no-driver-sweep' AND active
      AND (schedule ~ 'second' OR schedule = '* * * * *');
  r := r || public._qa_s13_ok('B9.5 no-driver sweep is scheduled autonomously at or below one minute',
        v_n = 1, (SELECT COALESCE(schedule,'not scheduled') FROM cron.job
