...
BEGIN
  ...
  n:=n+1; res:=res||jsonb_build_object('id',n,'name','a parallel staff role row is a canonical source, and only staff labels count','pass',
    public.admin_role_canonical(roles_only)='operations_admin'
  ...
  ok:=false; BEGIN PERFORM public.admin_review_approval(ap,'approved','self'); EXCEPTION WHEN OTHERS THEN ok:=true; END;
  ...
  ok:=false; BEGIN PERFORM public.admin_review_approval(ap,'approved','tmp'); EXCEPTION WHEN OTHERS THEN ok:=true; err:=SQLERRM; END;
  n:=n+1; res:=res||jsonb_build_object('id',n,'name','a temp-password god cannot approve','pass',
    ok AND err ILIKE '%approver_not_ready%');
  ...
  PERFORM set_config('request.jwt.claims', public._as_user_claims(fin), true);
  n:=n+1; res:=res||jsonb_build_object('id',n,'name','every operational capability is READ-only for finance','pass',
    NOT EXISTS (SELECT 1 FROM public.admin_capability_grants
                WHERE admin_role='finance_admin' AND capability LIKE 'ops.%' AND mode <> 'read')
    AND NOT coalesce(public.admin_capability('ops.orders.manage',fin),false)
    AND NOT coalesce(public.admin_capability('ops.drivers.manage',fin),false)
    AND NOT coalesce(public.admin_capability('ops.maps.manage',fin),false));
  v := public.ops_command_overview();
  n:=n+1; res:=res||jsonb_build_object('id',n,'name','finance reads operational facts without operational authority','pass',
    v IS NOT NULL AND NOT coalesce(public.admin_capability('ops.liveops.view',fin),false));
  ...
  n:=n+1; res:=res||jsonb_build_object('id',n,'name','the capability registry is guarded against session writes','pass',
    EXISTS (SELECT 1 FROM pg_trigger t JOIN pg_class c ON c.oid=t.tgrelid
            WHERE c.relname='admin_capability_grants' AND NOT t.tgisinternal)
    AND NOT has_table_privilege('authenticated','public.admin_capability_grants','INSERT')
    AND NOT has_table_privilege('authenticated','public.admin_capability_grants','UPDATE'));
  n:=n+1; res:=res||jsonb_build_object('id',n,'name','no session role can self-provision a staff account','pass',
    NOT has_table_privilege('authenticated','public.admin_users','INSERT')
    AND NOT has_table_privilege('anon','public.admin_users','INSERT')
    AND NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename='admin_users' AND cmd IN ('INSERT','ALL')));
  n:=n+1; res:=res||jsonb_build_object('id',n,'name','no session role can grant itself a role row','pass',
    NOT has_table_privilege('authenticated','public.user_roles','INSERT')
  ...
  -- ==================== G · AUDIT / PROVENANCE ============================
  n:=n+1; res:=res||jsonb_build_object('id',n,'name','the audit trail is append-only for every session role','pass',
    NOT has_table_privilege('authenticated','public.audit_logs','UPDATE')
    AND NOT has_table_privilege('authenticated','public.audit_logs','DELETE')
    AND NOT has_table_privilege('anon','public.audit_logs','UPDATE')
    AND NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename='audit_logs' AND cmd IN ('UPDATE','DELETE','ALL')));
  n:=n+1; res:=res||jsonb_build_object('id',n,'name','audit rows are written by the server, never by the browser','pass',
  ...
  n:=n+1; res:=res||jsonb_build_object('id',n,'name','staff authority derives from governance rows, never from a lane','pass',
  ...
  SELECT md5(coalesce(string_agg(id::text||balance_gnf||held_gnf||status::text,',' ORDER BY id),''))
    INTO wallets_after FROM public.wallets WHERE owner_user_id <> ALL(ids);
  ...
  n:=n+1; res:=res||jsonb_build_object('id',n,'name','zero fixture audit rows remain','pass',
    NOT EXISTS (SELECT 1 FROM public.audit_logs WHERE actor_user_id = ANY(ids)));
