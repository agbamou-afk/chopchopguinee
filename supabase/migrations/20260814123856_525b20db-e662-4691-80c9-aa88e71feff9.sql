...
DECLARE
  ...
  v_sum_total bigint; v_sum_m bigint; v_sum_d bigint; v_sum_p bigint;
...
BEGIN
  ...
  r := r || public._qa_s13_ok('S0.22 the photo verifier and hash primitives are closed to clients',
        ...
        AND NOT has_function_privilege('authenticated','public._repas_custody_hash(text,text)','EXECUTE')
        AND NOT has_function_privilege('anon','public._repas_custody_hash(text,text)','EXECUTE'), NULL);
  ...
  BEGIN
    ...
    SELECT COALESCE(sum(order_total_gnf),0), COALESCE(sum(merchant_credited_gnf),0),
           COALESCE(sum(driver_earning_gnf),0), COALESCE(sum(platform_revenue_gnf),0)
      INTO v_sum_total, v_sum_m, v_sum_d, v_sum_p
      FROM public.chop_pay_order_runtime
     WHERE source_module='repas' AND source_id IN (v_o1,v_o5) AND state='completed';
    SELECT count(*) INTO v_n FROM public.chop_pay_order_runtime
     WHERE source_module='repas' AND source_id IN (v_o1,v_o5) AND state='completed';
    r := r || public._qa_s13_ok('E1.0 both custody orders settled through one runtime each', v_n = 2, v_n::text);
    r := r || public._qa_s13_ok('E1.1 every franc the customer paid landed on a real counterparty',
          (v_c0 - v_c1) = (v_mm1 - v_mm0) + (v_d1 - v_d0) + (v_x1 - v_x0),
          (v_c0-v_c1)::text||' = '||(v_mm1-v_mm0)::text||'+'||(v_d1-v_d0)::text||'+'||(v_x1-v_x0)::text);
    r := r || public._qa_s13_ok('E1.1b the courier was paid exactly the settled earnings, never more',
          (v_d1 - v_d0) = v_sum_d AND v_sum_d = 15000, (v_d1 - v_d0)::text||'/'||v_sum_d::text);
    r := r || public._qa_s13_ok('E1.1c the platform took exactly the settled fees, never more',
          (v_x1 - v_x0) = v_sum_p AND v_sum_p = 3000, (v_x1 - v_x0)::text||'/'||v_sum_p::text);
    r := r || public._qa_s13_ok('E1.1d the merchant received at least the settled principal',
          (v_mm1 - v_mm0) >= v_sum_m AND v_sum_m = 300000, (v_mm1 - v_mm0)::text||'/'||v_sum_m::text);
    r := r || public._qa_s13_ok('E1.1e the settled orders decompose exactly into their parts',
          v_sum_total = v_sum_m + v_sum_d + v_sum_p,
          v_sum_total::text||' = '||v_sum_m::text||'+'||v_sum_d::text||'+'||v_sum_p::text);
