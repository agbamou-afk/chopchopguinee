...
DECLARE
  ...
  v_sub bigint; v_fee bigint; v_row public.food_orders%ROWTYPE;
...
BEGIN
  ...
  -- Pure derivation truth: no invented precision.
  ...
  BEGIN
    ...
    -- A client that lies about its source cannot claim verification.
    ...
    SELECT count(*) INTO v_n FROM public.food_orders o
      JOIN public.food_restaurants fr ON fr.id = o.restaurant_id
     WHERE fr.id = v_resto AND o.delivery_location_quality = 'unverifiable';
    ...
    -- A destination sent on a pickup order must never leak into the record.
    v_res := public.repas_order_create(v_resto,
        jsonb_build_array(jsonb_build_object('menu_item_id',v_item2,'qty',1)),
        'pickup','choppay', gen_random_uuid(),
    ...
    -- Fresh economics always come from the effective policy, never from the client.
    v_res := public.repas_order_create(v_resto,
        jsonb_build_array(jsonb_build_object('menu_item_id',v_item,'qty',1)),
        'delivery','choppay', gen_random_uuid(), 'Kipe', 9.5395, -13.6760, NULL, NULL, NULL, 'gps');
    SELECT * INTO v_row FROM public.food_orders WHERE id = (v_res->>'order_id')::uuid;
    ...
    -- A changed destination on the same key is a different intent: never swallowed.
    BEGIN
      PERFORM public.repas_order_create(v_resto,
        jsonb_build_array(jsonb_build_object('menu_item_id',v_item,'qty',1)),
    ...
    v_res := public.repas_merchant_transition(v_oRep, 'accept', NULL);
    v_res2 := public.repas_merchant_transition(v_oRep, 'accept', NULL);
    ...
    r := r || public._qa_s13_ok('G2.2 the order state is confirmed exactly once',
          (SELECT state::text FROM public.food_orders WHERE id = v_oRep) = 'confirmed', NULL);
    SELECT count(*) INTO v_n FROM public.repas_custody_credentials WHERE order_id = v_oRep;
    r := r || public._qa_s13_ok('G2.3 the repeated accept issued no duplicate custody credential',
          v_n <= 1, v_n::text);
    v_res := public.repas_merchant_transition(v_oRep, 'prepare', NULL);
    v_res2 := public.repas_merchant_transition(v_oRep, 'prepare', NULL);
    ...
    r := r || public._qa_s13_ok('G2.5 no lifecycle state was skipped or invented',
          (SELECT state::text FROM public.food_orders WHERE id = v_oRep) = 'preparing', NULL);

    -- Terminal truth survives a reconnect.
    PERFORM set_config('request.jwt.claims', public._as_user_claims(v_cust), true);
...
CREATE OR REPLACE FUNCTION public._qa_node3_repas_r11_conakry_hardening()
RETURNS jsonb LANGUAGE plpgsql SECURITY DEFINER SET search_path TO 'public' AS $function$
DECLARE v_out jsonb; BEGIN
  PERFORM set_config('app.repas_fixture_verified','1', true);
  v_out := public._qa_node3_repas_r11_conakry_hardening_fxcore();
  PERFORM set_config('app.repas_fixture_verified','', true);
  RETURN v_out; END; $function$;

REVOKE ALL ON FUNCTION public._qa_node3_repas_r11_conakry_hardening() FROM PUBLIC, anon, authenticated;
REVOKE ALL ON FUNCTION public._qa_node3_repas_r11_conakry_hardening_fxcore() FROM PUBLIC, anon, authenticated;
