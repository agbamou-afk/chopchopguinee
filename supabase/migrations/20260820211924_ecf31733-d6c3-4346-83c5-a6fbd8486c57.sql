-- Default privileges in this project grant CRUD on new public tables; strip them.
REVOKE ALL ON TABLE public.professional_identities FROM anon;
REVOKE ALL ON TABLE public.professional_identities FROM authenticated;
GRANT SELECT ON TABLE public.professional_identities TO authenticated;
GRANT ALL ON TABLE public.professional_identities TO service_role;
...
DECLARE
  ...
  v_json jsonb; v_err text; v_n int;
  f_ur int; f_w int;
  p6 boolean; p7 boolean; p8 boolean; p9 boolean; p10 boolean; p11 boolean; p12 boolean;
  d6 text; d7 text;
...
BEGIN
  ...
  -- fixture-side baseline (signup triggers may create roles/wallets on their own)
  ...
  r := r || public._qa_s13_ok('N5A2.D2 same-type re-claim is idempotent (same row)', v_row2.id = v_id1, NULL);
  ...
  BEGIN
    ...
    v_err := NULL;
  EXCEPTION WHEN others THEN v_err := SQLERRM; END;
  r := r || public._qa_s13_ok('N5A2.F1 signed-out read is denied (AUTH_REQUIRED)', v_err = 'AUTH_REQUIRED', v_err);
  ...
  -- runtime RLS / privilege proof under the authenticated role (assertions recorded after RESET)
  p6 := NULL; p7 := NULL; p8 := NULL; p9 := NULL; p10 := NULL; p11 := NULL; p12 := NULL;
  BEGIN
    PERFORM set_config('request.jwt.claims', public._as_user_claims(u3), true);
    ...
    BEGIN
      ...
      p9 := false;
    EXCEPTION WHEN others THEN p9 := true; END;
    BEGIN
      EXECUTE 'DELETE FROM public.professional_identities';
      p10 := false;
    EXCEPTION WHEN others THEN p10 := true; END;
    BEGIN
      EXECUTE 'TRUNCATE public.professional_identities';
      p11 := false;
    EXCEPTION WHEN others THEN p11 := true; END;
    BEGIN
      EXECUTE format('SELECT public._professional_identity_claim(%L,%L,%L)', u3,'driver','qa');
      p12 := false;
    EXCEPTION WHEN others THEN p12 := true; END;
    EXECUTE 'RESET ROLE';
  EXCEPTION WHEN others THEN
    EXECUTE 'RESET ROLE';
    d6 := COALESCE(d6,'probe error: '||SQLERRM);
  END;
  PERFORM set_config('request.jwt.claims', NULL, true);

  r := r || public._qa_s13_ok('N5A2.F6 authenticated user sees no other user rows', p6 IS TRUE, d6);
  r := r || public._qa_s13_ok('N5A2.F7 arbitrary other-user lookup by uuid returns nothing', p7 IS TRUE, d7);
  r := r || public._qa_s13_ok('N5A2.F8 authenticated direct INSERT denied', p8 IS TRUE, NULL);
  r := r || public._qa_s13_ok('N5A2.F9 authenticated direct UPDATE denied', p9 IS TRUE, NULL);
  r := r || public._qa_s13_ok('N5A2.F10 authenticated direct DELETE denied', p10 IS TRUE, NULL);
  r := r || public._qa_s13_ok('N5A2.F11 authenticated TRUNCATE denied', p11 IS TRUE, NULL);
  r := r || public._qa_s13_ok('N5A2.F12 authenticated cannot invoke the internal claim primitive', p12 IS TRUE, NULL);
  ...
  r := r || public._qa_s13_ok('N5A2.G6 claim granted no additional user_roles row (no merchant/driver role)',
        v_n = f_ur, 'after='||v_n||' fixture_baseline='||f_ur);
  SELECT count(*) INTO v_n FROM public.user_roles
   WHERE user_id = ANY(ids) AND role::text IN ('driver','merchant');
  r := r || public._qa_s13_ok('N5A2.G6b no driver/merchant role granted by an identity claim', v_n = 0, 'n='||v_n);
  SELECT count(*) INTO v_n FROM public.wallets WHERE owner_user_id = ANY(ids);
  r := r || public._qa_s13_ok('N5A2.G7 claim created no additional wallet',
        v_n = f_w, 'after='||v_n||' fixture_baseline='||f_w);
  r := r || public._qa_s13_ok('N5A2.G8 active identity does NOT imply driver approval',
        NOT EXISTS (SELECT 1 FROM public.driver_profiles d
                     WHERE d.user_id = ANY(ids) AND d.status::text = 'approved'), NULL);
  r := r || public._qa_s13_ok('N5A2.G9 active merchant identity does NOT imply an approved store',
        NOT EXISTS (SELECT 1 FROM public.merchant_stores s WHERE s.owner_user_id = ANY(ids)), NULL);
  ...
  r := r || public._qa_s13_ok('N5A2.H15 approved-store population unchanged', a_apr = b_apr, b_apr||'->'||a_apr);
  r := r || public._qa_s13_ok('N5A2.H16 feature flags unchanged', a_flags = b_flags, NULL);
  r := r || public._qa_s13_ok('N5A2.H17 no QA residue in professional_identities',
...
GRANT EXECUTE ON FUNCTION public._qa_node5_identity_a2() TO service_role, postgres;