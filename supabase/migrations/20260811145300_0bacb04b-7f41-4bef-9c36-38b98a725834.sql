CREATE TABLE IF NOT EXISTS public._qa_s11_results (
...
GRANT ALL ON public._qa_s11_results TO service_role;
ALTER TABLE public._qa_s11_results ENABLE ROW LEVEL SECURITY;
CREATE POLICY "God admins read qa s11" ON public._qa_s11_results
FOR SELECT TO authenticated USING (public.is_god_admin(auth.uid()));
...
DECLARE
  ...
  v_ref2 text := 'QA-S11B-' || substr(gen_random_uuid()::text,1,8);
  v_journal bigint; v_pass int; v_total int; v_sched1 jsonb; v_sched2 jsonb;
  PROCEDURE_DUMMY int;
BEGIN
  BEGIN
    -- ---------- fixtures ----------
    INSERT INTO public.merchant_stores (name, owner_user_id, phone)
    VALUES ('QA S11 Store', v_owner, '+224620000111') RETURNING id INTO v_store;
    ...
    -- ---------- merchant creates a settlement request ----------
    ...
    -- ---------- reference reuse on another order ----------
    PERFORM set_config('request.jwt.claims', public._as_user_claims(v_owner), true);
    ...
    END;
    BEGIN
      PERFORM public.payout_reject_release(v_o, 'merchant tries to release');
      r := r || jsonb_build_object('name','A24 merchant cannot release payouts','ok',false,'detail','allowed');
    EXCEPTION WHEN OTHERS THEN
      r := r || jsonb_build_object('name','A24 merchant cannot release payouts',
        'ok', SQLERRM LIKE 'NOT_AUTHORIZED%' OR SQLERRM LIKE 'ALREADY_SETTLED%','detail',SQLERRM);
    END;
  ...
  -- ---------- privilege assertions (no fixtures needed) ----------
  ...
  r := r || jsonb_build_object('name','A29 global ledger zero-sum','ok', v_sum = 0,
    'detail', format('sum=%s', v_sum));

  SELECT COALESCE(balance_gnf,0) INTO v_master FROM public.wallets WHERE party_type='master' LIMIT 1;
  r := r || jsonb_build_object('name','A30 master wallet baseline unchanged','ok', v_master = -100435,
    'detail', format('master=%s', v_master));

  SELECT COALESCE(bool_and(NOT enabled), true) INTO v_status FROM public.feature_flags
   WHERE key IN ('merchant_om_settlement_enabled','driver_cashout_enabled','chop_pay_p2p_enabled');
  r := r || jsonb_build_object('name','A31 stage flags remain OFF after run','ok', v_status::boolean,
    'detail', format('all_off=%s', v_status));

  SELECT count(*) INTO v_n FROM public.payout_orders;
  r := r || jsonb_build_object('name','A32 no QA rows persisted','ok', v_n = 0,
