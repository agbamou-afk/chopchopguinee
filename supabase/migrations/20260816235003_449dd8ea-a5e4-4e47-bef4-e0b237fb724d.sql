...
BEGIN
  FOREACH s IN ARRAY ARRAY[
    ...
    '_qa_node3_repas_r6_custody','_qa_node3_repas_r7_tracking_receipt','_qa_node3_repas_r8_discovery_truth',
...
DECLARE t int; f int;
BEGIN
  CREATE TEMP TABLE _r5e AS SELECT * FROM public._qa_node3_repas_r5();
  SELECT count(*) INTO t FROM _r5e;
  EXECUTE 'SELECT count(*) FROM _r5e WHERE NOT (' ||
    (SELECT string_agg(quote_ident(column_name),' ') FROM information_schema.columns
      WHERE table_name='_r5e' AND data_type='boolean' LIMIT 1) || ')' INTO f;
...
SELECT jsonb_build_object(
  ...
  'anon_has_role', has_function_privilege('anon','public.has_role(uuid,app_role)','EXECUTE'),
  'r7_residue', (SELECT count(*) FROM public.food_restaurants WHERE slug LIKE 'qa-n3r7-%'),
  'chop_pay_flags', (SELECT jsonb_object_agg(key, enabled) FROM public.feature_flags WHERE key LIKE 'chop_pay%')
) AS board;