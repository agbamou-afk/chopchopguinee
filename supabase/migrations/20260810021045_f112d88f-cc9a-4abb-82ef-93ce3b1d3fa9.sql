CREATE OR REPLACE FUNCTION public._qa_s4_ok(p_name text, p_cond boolean, p_detail text DEFAULT NULL)
RETURNS jsonb LANGUAGE sql IMMUTABLE SET search_path = public AS $$
  SELECT jsonb_build_object('n', p_name, 'ok', COALESCE(p_cond,false), 'd', p_detail)
$$;
REVOKE ALL ON FUNCTION public._qa_s4_ok(text,boolean,text) FROM PUBLIC, anon, authenticated;
...
DECLARE
  ...
  v_ms uuid; v_ms2 uuid; v_ms3 uuid; v_ms4 uuid; v_ms5 uuid; v_ms6 uuid; v_ms7 uuid; v_ms8 uuid;
...
BEGIN
  ...
  BEGIN
    ...
    -- custody then canonical dropoff
    PERFORM set_config('request.jwt.claims', public._as_user_claims(v_drv), true);
    BEGIN
    ...
    -- ============================================================
    -- B1 insufficient unrestricted funds -> no partial assignment
    INSERT INTO public.food_orders(user_id, restaurant_id, fulfillment, payment_method, subtotal_gnf)
    ...
    -- B2 direct state write on a cash order is denied purely on payment_method
    BEGIN
    ...
    -- B3 non-cash order lifecycle unchanged
    INSERT INTO public.food_orders(user_id, restaurant_id, fulfillment, payment_method, subtotal_gnf)
    ...
    -- B4 flag OFF blocks cash acceptance with full rollback
    ...
    -- merchant rejection before funding
    INSERT INTO public.food_orders(user_id, restaurant_id, fulfillment, payment_method, subtotal_gnf)
    ...
    r := r || public._qa_s4_ok('E7 Marche completion replay adds zero',
      v_j->>'status' = 'already_completed' AND v_b2 = v_b);
    SELECT COALESCE(SUM(amount_gnf),0) INTO v_n FROM public.ledger_postings WHERE source_id = v_off;
    r := r || public._qa_s4_ok('E8 Marche ledger drift 0', v_n = 0);

    -- ============================================================
    -- F. POST-PREPARATION DISPUTE ECONOMICS
    -- ============================================================
    -- F/1 release_driver_funding
    INSERT INTO public.food_orders(user_id, restaurant_id, fulfillment, payment_method, subtotal_gnf)
    ...
    -- beneficiary mismatch first (must not move value)
    SELECT balance_gnf INTO v_mwbal FROM public.wallets WHERE owner_user_id=v_own AND party_type='merchant';
    BEGIN
    ...
    r := r || public._qa_s4_ok('F2 release_driver_funding restores captured principal to the driver',
      ...
      format('driver=%s(+150000?) held=%s payable=%s', v_b, v_n, v_txt));
    SELECT balance_gnf INTO v_b2 FROM public.wallets WHERE party_type='master';
    ...
    -- F/4 unrecoverable payable -> reconciliation required, nothing mutated
    ...
    -- unauthorised dispute resolution attempt
    PERFORM set_config('request.jwt.claims', public._as_user_claims(v_cust), true);
    BEGIN
    ...
    SELECT count(*) INTO v_n FROM public.ledger_postings p
     WHERE p.source_id IN (v_fo, v_fo4, v_fo5, v_fo6, v_fo7, v_fo8, v_off)
       AND p.source_module NOT IN ('repas','marche');
    r := r || public._qa_s4_ok('H3 source attribution exact on every posting', v_n = 0);
