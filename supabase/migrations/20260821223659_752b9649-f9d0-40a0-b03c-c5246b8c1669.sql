-- QA cleanup must be able to purge fixture liability rows before wallets.
...
AS $function$
BEGIN
  DELETE FROM public.dormant_closed_account_liabilities WHERE user_id = ANY(p_ids);
  ...
  DELETE FROM public.wallet_transactions
   WHERE related_user_id = ANY(p_ids)
      OR from_wallet_id IN (SELECT id FROM public.wallets WHERE owner_user_id = ANY(p_ids))
...
BEGIN
  ...
  VALUES (u_pos,'driver','active',now());
