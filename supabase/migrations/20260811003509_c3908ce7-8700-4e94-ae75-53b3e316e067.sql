CREATE OR REPLACE FUNCTION public._qa_s6_g4(p_drv uuid)
RETURNS boolean LANGUAGE sql STABLE SECURITY DEFINER SET search_path TO 'public' AS $$
  SELECT NOT EXISTS (
    SELECT 1 FROM public.wallet_transactions t
     JOIN public.wallets w ON w.id = t.to_wallet_id
    WHERE w.owner_user_id = p_drv AND w.party_type = 'driver' AND t.amount_gnf = 100000);
$$;
REVOKE ALL ON FUNCTION public._qa_s6_g4(uuid) FROM PUBLIC, anon, authenticated;
...
BEGIN
  ...
  BEGIN
    ...
    r := r || public._qa_s5_ok('G4 no digital delivery-fee earning on a cash shipment', public._qa_s6_g4(v_drv));
