CREATE OR REPLACE FUNCTION public._qa_s3b_guards() RETURNS jsonb
LANGUAGE plpgsql SET search_path TO 'public'
AS $fn$
DECLARE res jsonb := '{}'::jsonb;
BEGIN
  BEGIN
    PERFORM public.wallet_internal_transfer(gen_random_uuid(),'driver',NULL,'master',1000,'qa',NULL);
    res := res || jsonb_build_object('A.guard.direct_transfer_blocked','FAIL');
  EXCEPTION WHEN OTHERS THEN
    res := res || jsonb_build_object('A.guard.direct_transfer_blocked','PASS','A.guard.err',left(SQLERRM,80));
  END;
  BEGIN
    PERFORM public.ride_accept(gen_random_uuid());
    res := res || jsonb_build_object('B.guard.direct_ride_accept_blocked','FAIL');
  EXCEPTION WHEN OTHERS THEN
    res := res || jsonb_build_object('B.guard.direct_ride_accept_blocked','PASS','B.guard.err',left(SQLERRM,80));
  END;
  BEGIN
    PERFORM public.ride_dispatch(gen_random_uuid());
    res := res || jsonb_build_object('D.guard.direct_dispatch_blocked','FAIL');
  EXCEPTION WHEN OTHERS THEN
    res := res || jsonb_build_object('D.guard.direct_dispatch_blocked','PASS','D.guard.err',left(SQLERRM,80));
  END;
  res := res || jsonb_build_object('S2.effective_role', current_user);
  RAISE EXCEPTION 'QA_S3B_GUARDS %', res::text;
END $fn$;

REVOKE ALL ON FUNCTION public._qa_s3b_guards() FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public._qa_s3b_guards() TO authenticated;
...
BEGIN
  ...
  res := res || jsonb_build_object(
    ...
    'S2.guard_bodies','tested_out_of_band', 'S2.owner', v_owner);
