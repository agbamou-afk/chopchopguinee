...
DECLARE
  ...
  res jsonb; v_txt text; v_n bigint; v_bal bigint; v_stf_base bigint;
  fake text;
  a12_fns text[] := ARRAY[
...
BEGIN
  ...
  ON CONFLICT DO NOTHING;
  SELECT count(*) INTO v_stf_base FROM public.user_roles WHERE user_id=u_stf;
  ...
  -- historical operational role rows, as a real approved driver would carry
  ...
  PERFORM set_config('request.jwt.claims', NULL, true);
  SELECT count(*) INTO v_n FROM public.user_roles WHERE user_id=u_stf;
  r := r || public._qa_s13_ok('N5A12.C3 staff role revocation removes exactly one row',
        (res->>'removed')::int = 1 AND v_n = v_stf_base - 1, res::text||' rows='||v_n);
  r := r || public._qa_s13_ok('N5A12.C4 revoked staff role no longer satisfies has_role',
        ...
        AND NOT EXISTS (SELECT 1 FROM public.user_roles
                         WHERE user_id=u_stf AND role='field_captain'::public.app_role), NULL);
  ...
  END;
  r := r || public._qa_s13_ok('N5A12.C6b the refused revocation left the professional role row intact',
        public.has_role(u_drv,'driver'::public.app_role), NULL);
  BEGIN
    PERFORM set_config('request.jwt.claims', public._as_user_claims(u_god), true);
  ...
  EXCEPTION WHEN others THEN
    ...
    r := r || public._qa_s13_ok('N5A12.C7 governance tooling refuses to grant a professional role',
          SQLERRM = 'PROFESSIONAL_ROLE_NOT_GOVERNANCE', SQLERRM);
  END;
  r := r || public._qa_s13_ok('N5A12.C8 the refused grant left no professional role row behind',
        NOT EXISTS (SELECT 1 FROM public.user_roles
                     WHERE user_id=u_stf AND role='merchant'::public.app_role), NULL);
  PERFORM set_config('request.jwt.claims', public._as_user_claims(u_god), true);
  ...
  r := r || public._qa_s13_ok('N5A12.D10 the historical driver role row is preserved and non-authoritative',
        EXISTS (SELECT 1 FROM public.user_roles
                 WHERE user_id=u_drv AND role='driver'::public.app_role)
        AND public.has_role(u_drv,'driver'::public.app_role)
  ...
  r := r || public._qa_s13_ok('N5A12.H7 the retained driver role row grants no professional class',
        EXISTS (SELECT 1 FROM public.user_roles
                 WHERE user_id=u_ad AND role='driver'::public.app_role)
  ...
  r := r || public._qa_s13_ok('N5A12.H10 class helpers never read request claims for authority',
        (SELECT bool_and(prosrc !~ 'request\.jwt')
  ...
  r := r || public._qa_s13_ok('N5A12.H11 class helpers resolve against the canonical identity table',
        (SELECT bool_and(prosrc ~ 'professional_identities')
           FROM pg_proc p JOIN pg_namespace n ON n.oid=p.pronamespace
          WHERE n.nspname='public'
            AND p.proname IN ('_driver_class_active','_merchant_class_active','professional_active_type'))
        AND (SELECT prosrc ~ 'professional_active_type'
               FROM pg_proc p JOIN pg_namespace n ON n.oid=p.pronamespace
              WHERE n.nspname='public' AND p.proname='account_available_modes'), NULL);
  ...
  SELECT count(*) INTO v_n FROM public.wallets WHERE owner_user_id=u_fin AND party_type='driver';
  r := r || public._qa_s13_ok('N5A12.I9 the offboarded professional wallet is neither deleted nor duplicated',
        v_n = 1, 'driver_wallets='||v_n);
