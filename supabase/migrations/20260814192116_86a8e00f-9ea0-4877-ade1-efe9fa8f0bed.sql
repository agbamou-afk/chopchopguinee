ALTER FUNCTION public._qa_node3_repas_r8_discovery() RENAME TO _qa_node3_repas_r8_core;
...
DECLARE
  ...
  v_orders0 int; v_missions0 int; v_journals0 int;
  v_flag_prev boolean;
BEGIN
  ...
  r := r || public._qa_s13_ok('P12.0 canonical publication assertion helper exists',
        to_regprocedure('public._repas_assert_orderable_publication(record)') IS NOT NULL
  ...
  r := r || public._qa_s13_ok('P12.7 restaurant INSERT policy exists (anon cannot self-serve)',
        EXISTS (SELECT 1 FROM pg_policies WHERE schemaname='public'
                 AND tablename='food_restaurants' AND cmd='INSERT'), NULL);
  r := r || public._qa_s13_ok('P12.8 restaurant INSERT policy is scoped to the authenticated owner',
        (SELECT coalesce(bool_and(with_check LIKE '%auth.uid()%'), false) FROM pg_policies
          WHERE schemaname='public' AND tablename='food_restaurants' AND cmd='INSERT'), NULL);
  r := r || public._qa_s13_ok('P12.9 anon holds no INSERT grant on restaurants',
        NOT has_table_privilege('anon','public.food_restaurants','INSERT'), NULL);
  r := r || public._qa_s13_ok('P12.10 anon holds no INSERT grant on menu items',
        NOT has_table_privilege('anon','public.food_menu_items','INSERT'), NULL);

  BEGIN
    SELECT count(*) INTO v_orders0 FROM public.food_orders;
    SELECT count(*) INTO v_missions0 FROM public.missions;
    SELECT count(*) INTO v_journals0 FROM public.ledger_journals;
    ...
    SELECT count(*) INTO v_n FROM public.wallet_transactions WHERE wallet_id IN
      (SELECT id FROM public.wallets WHERE user_id = v_cust);
    ...
    -- all items unavailable must still be publishable
    ...
    r := r || public._qa_s13_ok('P15.1 a stranger sees no row to update on another restaurant',
          NOT EXISTS (SELECT 1 FROM pg_policies WHERE schemaname='public'
                       AND tablename='food_restaurants' AND cmd='UPDATE'
                       AND qual NOT LIKE '%auth.uid()%' AND qual NOT LIKE '%has_role%'
                       AND qual NOT LIKE '%_is_%'), NULL);
    r := r || public._qa_s13_ok('P15.2 menu mutation policies are owner/staff scoped',
          (SELECT coalesce(bool_and(coalesce(qual,with_check) LIKE '%auth.uid()%'
                                    OR coalesce(qual,with_check) LIKE '%has_role%'
                                    OR coalesce(qual,with_check) LIKE '%_is_%'), false)
    ...
    BEGIN
      ...
      v_err := 'NO_ERROR';
    EXCEPTION WHEN OTHERS THEN v_err := SQLERRM; END;
    r := r || public._qa_s13_ok('P15.3 stranger cannot rename another restaurant (guard or RLS)',
    ...
    r := r || public._qa_s13_ok('P16.1 restaurant SELECT policy hides non-verified rows',
          (SELECT coalesce(bool_and(qual LIKE '%verified%'), false) FROM pg_policies
            WHERE schemaname='public' AND tablename='food_restaurants' AND cmd='SELECT'
              AND policyname NOT ILIKE '%own%' AND policyname NOT ILIKE '%staff%'
              AND policyname NOT ILIKE '%admin%'), NULL);
    r := r || public._qa_s13_ok('P16.2 menu SELECT policy hides non-verified supply',
          (SELECT coalesce(bool_and(qual LIKE '%verified%'), false) FROM pg_policies
            WHERE schemaname='public' AND tablename='food_menu_items' AND cmd='SELECT'
              AND policyname NOT ILIKE '%own%' AND policyname NOT ILIKE '%staff%'
              AND policyname NOT ILIKE '%admin%'), NULL);
    ...
    -- current supply mutates and the restaurant is pulled from the market
...
AS $function$
DECLARE
  v_core jsonb; v_extra jsonb; v_all jsonb;
BEGIN
  v_core := public._qa_node3_repas_r8_core();
  v_extra := public._qa_node3_repas_r8_extra();
  v_all := coalesce(v_core->'results','[]'::jsonb) || coalesce(v_extra,'[]'::jsonb);
...
REVOKE ALL ON FUNCTION public._qa_node3_repas_r8_core() FROM PUBLIC, anon, authenticated;
