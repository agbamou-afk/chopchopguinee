...
DECLARE
  ...
  v_store uuid; v_rest uuid; v_o1 uuid; v_o2 uuid; v_o3 uuid; v_mis uuid;
...
BEGIN
  ...
  BEGIN
    ...
    INSERT INTO public.merchant_stores(owner_user_id, name, status)
    VALUES (v_own,'QA S13 Store','approved') RETURNING id INTO v_store;
    INSERT INTO public.food_restaurants(slug, name, merchant_store_id, owner_user_id, status)
    VALUES ('qa-s13-'||substr(v_store::text,1,8),'QA S13 Resto', v_store, v_own, 'approved')
    ...
    -- kitchen cannot start before funding is secured
    ...
    -- restricted starting credit can never fund merchandise principal
    ...
    -- funded acceptance by a courier with unrestricted funds
    ...
    SELECT COALESCE(sum(promo_gnf),0) INTO v_n FROM public.mission_financial_holds WHERE source_id = v_o1;
    ...
    -- customer cannot cancel once the kitchen is preparing
    PERFORM set_config('request.jwt.claims', public._as_user_claims(v_own), true);
    ...
    -- completion: fee captured once, duplicates inert
    ...
    -- stage gate: turning the checkout rail OFF blocks new authorizations
    INSERT INTO public.food_orders(user_id, restaurant_id, subtotal_gnf, payment_method, state, fulfillment)
    ...
    -- ledger integrity across both tenders
