...
BEGIN
  ...
  BEGIN
    ...
    -- H. CANCEL AFTER PREPARATION DENIED + I. DISPUTE complete_as_delivered
    INSERT INTO public.food_orders(user_id,restaurant_id,fulfillment,payment_method,subtotal_gnf)
      VALUES (v_cust,v_rest,'delivery','choppay',150000) RETURNING id INTO v_fo;
    INSERT INTO public.missions(type,customer_id,ref_food_order_id,estimated_earning_gnf,merchant_store_id)
      VALUES ('food_delivery',v_cust,v_fo,25000,v_store) RETURNING id INTO v_ms;
    ...
    -- custody must be established by the assigned courier before any delivered outcome
    PERFORM set_config('request.jwt.claims', public._as_user_claims(v_drv), true);
    ...
    SELECT state INTO v_txt FROM public.chop_pay_order_runtime WHERE source_id=v_fo;
    r := r || public._qa_s5_ok('J5 runtime resolved out of dispute', v_txt IN ('refunded','dispute_resolved'), v_txt);
    ...
    -- L0. PREPARATION IS DENIED BEFORE MERCHANDISE FUNDING
    INSERT INTO public.food_orders(user_id,restaurant_id,fulfillment,payment_method,subtotal_gnf)
      VALUES (v_cust2,v_rest,'delivery','choppay',150000) RETURNING id INTO v_fo;
    INSERT INTO public.missions(type,customer_id,ref_food_order_id,estimated_earning_gnf,merchant_store_id)
      VALUES ('food_delivery',v_cust2,v_fo,25000,v_store) RETURNING id INTO v_ms;
    ...
    BEGIN
      PERFORM public.chop_pay_merchant_prepare('repas', v_fo);
      r := r || public._qa_s5_ok('L0 preparation is denied before merchandise funding is captured', false,'no error');
    EXCEPTION WHEN OTHERS THEN
      GET STACKED DIAGNOSTICS v_err = MESSAGE_TEXT;
      r := r || public._qa_s5_ok('L0 preparation is denied before merchandise funding is captured',
        v_err LIKE 'PREPARATION_REQUIRES_FUNDED_ORDER%' OR v_err LIKE 'MERCHANDISE_FUNDING_NOT_SECURED%', v_err);
    END;
    SELECT state INTO v_txt FROM public.chop_pay_order_runtime WHERE source_id=v_fo;
    r := r || public._qa_s5_ok('L0b runtime stays in accepted state after the denied preparation',
      v_txt='accepted', v_txt);
...
REVOKE ALL ON FUNCTION public._qa_s5_run2() FROM PUBLIC, anon, authenticated;