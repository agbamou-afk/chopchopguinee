...
DECLARE
  ...
  v_o uuid; v_onc uuid; v_oneg uuid; v_ord public.marche_orders;
  ...
  v_keys text; v_now timestamptz := now(); v_base timestamptz; v_raw bigint; v_der bigint;
...
BEGIN
  ...
  r := r || public._qa_s13_ok('N4R35.A21 observation guard covers creation, not only edits',
        (SELECT bool_and(pg_get_triggerdef(oid) LIKE '%BEFORE INSERT OR UPDATE OR DELETE%')
           FROM pg_trigger WHERE tgrelid='public.marche_fulfillment_observations'::regclass
            AND NOT tgisinternal), NULL);
  ...
  r := r || public._qa_s13_ok('N4R35.C31 ORDER_COMMITTED is the only production-wired milestone',
        (SELECT count(*) FROM pg_proc WHERE pronamespace='public'::regnamespace
  ...
  BEGIN
    ...
    -- deterministic baseline: the stored commitment truth, not a loosely captured now()
    SELECT occurred_at INTO v_base FROM public.marche_fulfillment_events
      WHERE order_id = v_o AND event_type = 'ORDER_COMMITTED';
    ...
    r := r || public._qa_s13_ok('N4R35.D11 fulfillment mode is explicit server truth, not client-authored',
          p.fulfillment_mode = 'unspecified' AND p.fulfillment_mode_source = 'no_authoritative_dispatch_decision',
          p.fulfillment_mode);
    ...
    PERFORM public.marche_fulfillment_event_append(v_o,'MERCHANT_ACCEPTED', v_base, 'qa_harness', v_o::text, 'accept-1','merchant');
    PERFORM public.marche_fulfillment_event_append(v_o,'MERCHANT_ACCEPTED', v_base + interval '5 minutes', 'qa_harness', v_o::text, 'accept-1','merchant');
    ...
    r := r || public._qa_s13_ok('N4R35.G2 first-write history is not rewritten',
          (SELECT occurred_at FROM public.marche_fulfillment_events
            WHERE order_id = v_o AND event_type='MERCHANT_ACCEPTED') = v_base, NULL);
    ...
    -- derivation authority must NOT leak from the recomputation that just ran
    BEGIN
    ...
    BEGIN
      PERFORM set_config('marche.fulfillment_derive_token','1', true);
      INSERT INTO public.marche_fulfillment_observations(order_id, metric_name, duration_seconds,
        start_event_at, end_event_at, merchant_store_id, fulfillment_mode, distance_bucket,
        basket_units, distinct_products, basket_bucket, observed_at)
      VALUES (v_o,'COMMIT_TO_DELIVERED', 1, now(), now(), v_store, 'delivery','0-1km',1,1,'single', now());
      v_err := 'no-error';
    EXCEPTION WHEN OTHERS THEN v_err := SQLERRM; END;
    r := r || public._qa_s13_ok('N4R35.G8 a guessed derivation token cannot fabricate an observation',
          v_err = 'FULFILLMENT_OBSERVATION_DERIVED_ONLY', v_err);
    PERFORM set_config('marche.fulfillment_derive_token','', true);
    BEGIN
      UPDATE public.marche_fulfillment_observations SET duration_seconds = 7
       WHERE order_id = v_o AND metric_name='COMMIT_TO_MERCHANT_ACCEPTED';
      v_err := 'no-error';
    EXCEPTION WHEN OTHERS THEN v_err := SQLERRM; END;
    r := r || public._qa_s13_ok('N4R35.G9 derived observations cannot be edited after the fact',
          v_err = 'FULFILLMENT_OBSERVATION_DERIVED_ONLY', v_err);
    ...
    PERFORM public.marche_fulfillment_event_append(v_o,'MERCHANT_READY', v_base + interval '600 seconds','qa_harness', v_o::text,'ready-1','merchant');
    ...
    PERFORM public.marche_fulfillment_event_append(v_o,'COURIER_ENGAGED', v_base + interval '700 seconds','qa_harness', v_o::text,'eng-1','courier');
    PERFORM public.marche_fulfillment_event_append(v_o,'COURIER_AT_STORE', v_base + interval '1000 seconds','qa_harness', v_o::text,'atstore-1','courier');
    ...
    PERFORM public.marche_fulfillment_event_append(v_o,'SHOPPING_STARTED', v_base + interval '1010 seconds','qa_harness', v_o::text,'shop-s','courier');
    PERFORM public.marche_fulfillment_event_append(v_o,'SHOPPING_COMPLETED', v_base + interval '1460 seconds','qa_harness', v_o::text,'shop-e','courier');
    ...
    PERFORM public.marche_fulfillment_event_append(v_o,'PICKED_UP', v_base + interval '1500 seconds','qa_harness', v_o::text,'pick-1','courier');
    PERFORM public.marche_fulfillment_event_append(v_o,'DELIVERED', v_base + interval '2400 seconds','qa_harness', v_o::text,'deliv-1','courier');
    ...
    SELECT floor(EXTRACT(EPOCH FROM (
             (SELECT min(occurred_at) FROM public.marche_fulfillment_events
               WHERE order_id = v_o AND event_type='DELIVERED')
           - (SELECT min(occurred_at) FROM public.marche_fulfillment_events
               WHERE order_id = v_o AND event_type='ORDER_COMMITTED'))))::bigint INTO v_raw;
    SELECT duration_seconds INTO v_der FROM public.marche_fulfillment_observations
      WHERE order_id = v_o AND metric_name='COMMIT_TO_DELIVERED';
    r := r || public._qa_s13_ok('N4R35.H10 raw commit->delivered endpoint delta is exactly 2400s',
          v_raw = 2400, COALESCE(v_raw::text,'null'));
    r := r || public._qa_s13_ok('N4R35.H10b derived COMMIT_TO_DELIVERED equals the raw endpoint delta',
          v_der IS NOT NULL AND v_der = v_raw, COALESCE(v_der::text,'null'));
    ...
    PERFORM public.marche_fulfillment_event_append(v_o,'DELIVERED', v_base + interval '9999 seconds','qa_harness', v_o::text,'deliv-2','courier');
  ...
  r := r || public._qa_s13_ok('N4R35.S21 derivation authority token does not survive the pass',
        COALESCE(current_setting('marche.fulfillment_derive_token', true),'') = '',
        COALESCE(current_setting('marche.fulfillment_derive_token', true),''));
