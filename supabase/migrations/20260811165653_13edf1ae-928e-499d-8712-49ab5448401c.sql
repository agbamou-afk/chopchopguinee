...
BEGIN
  ...
  BEGIN
    ...
    -- cross-actor denial on a live cash order
    PERFORM set_config('request.jwt.claims', public._as_user_claims(v_dpromo), true);
    BEGIN v_res := public.cash_order_merchant_prepare('repas', v_o1); v_err := 'NO_ERROR';
    EXCEPTION WHEN OTHERS THEN v_err := SQLERRM; END;
    ...
    r := r || public._qa_s13_ok('C1.2 delivery fee excluded from the collateral basis',
      COALESCE((v_q->>'collateral_basis_gnf')::bigint,
               (v_q->>'merchandise_subtotal_gnf')::bigint) = 200000, v_q::text);
    ...
    PERFORM set_config('request.jwt.claims', public._as_user_claims(v_cust), true);
    v_q := public.cash_order_quote('marche', v_m1);
    r := r || public._qa_s13_ok('D1.1 Marche courier funds 100% of merchandise (150000)',
      (v_q->>'cash_funding_gnf')::bigint = 150000, v_q::text);
    ...
    r := r || public._qa_s13_ok('D1.3 Marche customer cash due = 150000 + 25000 + 1500 = 176500',
      (v_q->>'cash_due_gnf')::bigint = 176500, v_q->>'cash_due_gnf');
    ...
    EXCEPTION WHEN OTHERS THEN v_err := SQLERRM; END;
    r := r || public._qa_s13_ok('D3.1 Marche merchant rejection before preparation succeeds',
    ...
    PERFORM set_config('request.jwt.claims', public._as_user_claims(v_cust), true);
    v_q := public.chop_pay_quote('marche', v_m2);
    r := r || public._qa_s13_ok('E1.1 Marche Chop Pay order total = 176500 (canonical anchor)',
      (v_q->>'order_total_gnf')::bigint = 176500, v_q::text);
    ...
    r := r || public._qa_s13_ok('E1.3 Marche Chop Pay courier collateral = 50% of merchandise (75000)',
      (v_q->>'collateral_gnf')::bigint = 75000, v_q->>'collateral_gnf');
    ...
    SELECT COALESCE(balance_gnf,0) INTO v_mw FROM public.wallets
      WHERE owner_user_id = v_own AND party_type = 'merchant';
    r := r || public._qa_s13_ok('E3.1 merchant captured exactly the merchandise amount (150000)',
      v_err = 'NO_ERROR' AND v_mw - v_mw0 = 150000, format('%s %s -> %s', v_err, v_mw0, v_mw));
