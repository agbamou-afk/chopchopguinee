...
REVOKE ALL ON FUNCTION public._qa_node5_fr_seed(uuid[]) FROM anon, authenticated;
REVOKE ALL ON FUNCTION public._qa_node5_fr_profiles(uuid[]) FROM anon, authenticated;
REVOKE ALL ON FUNCTION public._qa_node5_identity_final_remediation() FROM anon, authenticated;
...
DECLARE
  ...
  u_liv uuid := gen_random_uuid();
  u_re  uuid := gen_random_uuid();
  u_god uuid := gen_random_uuid();
  ids uuid[];
  s_m uuid := gen_random_uuid();
  ride_id uuid := gen_random_uuid();
  ph text; ph_re text;
  res jsonb; v_txt text; v_bal bigint;
...
BEGIN
  ...
  r := r || public._qa_s13_ok('N5FR.A15 own-ride reads are routed through the access gate',
        (SELECT count(*) FROM pg_policies WHERE schemaname='public' AND tablename='rides'
          AND COALESCE(qual,'') LIKE '%auth_uid_active%') >= 2, NULL);
  ...
  -- ============ GOVERNED IDEMPOTENT RECONCILIATION ============
  ...
  -- ============ ACCESS TERMINATION + STALE CONTEXT ============
  ...
  r := r || public._qa_s13_ok('N5FR.G1 the access gate refuses a closed but authenticated caller',
        public.auth_uid_active() IS NULL, NULL);
  ...
  r := r || public._qa_s13_ok('N5FR.G7 the termination contract records source provenance',
        (SELECT count(*) FROM public.account_access_terminations
          WHERE user_id IN (u_d,u_m,u_x,u_fin) AND source IS NOT NULL) = 4, NULL);
  ...
  DELETE FROM public.wallet_transactions WHERE wallet_id IN
    (SELECT id FROM public.wallets WHERE owner_user_id = ANY(ids));
