DROP FUNCTION IF EXISTS public._qa_node3_repas_r10_operations_fxcore_v2();
...
BEGIN
  ...
  BEGIN
    ...
    SELECT count(*) INTO v_n FROM public.repas_ops_events WHERE food_order_id = v_oA;
    r := r || public._qa_s13_ok('P3.13 note and contact commands each append one event', v_n = 4, v_n::text);
    BEGIN PERFORM public.repas_ops_command(v_oA,'contact_courier',gen_random_uuid(),NULL,'QA R10 livreur appele');
      v_err := 'NO_ERROR';
    EXCEPTION WHEN OTHERS THEN v_err := SQLERRM; END;
    r := r || public._qa_s13_ok('P3.13b contacting a courier is unavailable while none is assigned',
          v_err LIKE '%ACTION_NOT_ALLOWED%', v_err);
    SELECT count(*) INTO v_n FROM public.repas_ops_events WHERE food_order_id = v_oA;
    r := r || public._qa_s13_ok('P3.13c the refused contact appended nothing', v_n = 4, v_n::text);
    ...
    r := r || public._qa_s13_ok('P3.16 a direct delete of a prior ops event is refused',
          v_err LIKE '%OPS_EVENTS_APPEND_ONLY%', v_err);
    SELECT count(*) INTO v_n FROM public.repas_ops_events WHERE food_order_id = v_oA;
    r := r || public._qa_s13_ok('P3.17 the timeline survived the tamper attempts intact', v_n = 5, v_n::text);
    ...
    r := r || public._qa_s13_ok('P3.19 the merged timeline includes ops interventions',
          (SELECT count(*) FROM jsonb_array_elements(v_d->'timeline') e
            WHERE e->>'source' = 'ops') = 5, v_d->>'timeline');
