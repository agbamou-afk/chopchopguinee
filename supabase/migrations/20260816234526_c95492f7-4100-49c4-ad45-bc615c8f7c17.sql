...
DECLARE t int; f int;
BEGIN
  CREATE TEMP TABLE _r5d AS SELECT * FROM public._qa_node3_repas_r5();
  SELECT count(*) INTO t FROM _r5d;
  EXECUTE 'SELECT count(*) FROM _r5d WHERE NOT (' ||
    (SELECT string_agg(quote_ident(column_name),' ') FROM information_schema.columns
      WHERE table_name='_r5d' AND data_type='boolean' LIMIT 1) || ')' INTO f;
...
SELECT jsonb_build_object(
  ...
  'flags_chop_pay', (SELECT jsonb_object_agg(key, enabled) FROM public.feature_flags WHERE key like 'chop_pay%'),
  'anon_has_role', has_function_privilege('anon','public.has_role(uuid,app_role)','EXECUTE'),
  'direct_marche_grants', (SELECT count(*) FROM information_schema.role_table_grants
     WHERE table_schema='public' AND grantee IN ('anon','authenticated')
       AND table_name LIKE 'marche%')
) AS board;