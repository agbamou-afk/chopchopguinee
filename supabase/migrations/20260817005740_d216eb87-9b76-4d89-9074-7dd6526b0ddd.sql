...
DECLARE
  r jsonb := '[]'::jsonb;
  v_err text; v_n bigint; v_num numeric; v_json jsonb; v_j2 jsonb;
  ...
  v_r1 uuid; v_r3 uuid; v_r4 uuid;
  v_a1 uuid; v_a2 uuid;
  v_b1 jsonb; v_b2 jsonb;
  v_bal bigint; v_held bigint;
BEGIN
  ...
  BEGIN
    ...
    r := r || public._qa_s13_ok('N4R65.B6 the fallback basis is an explicit customer-declared ceiling',
          v_json->>'estimate_basis' = 'customer_declared_ceiling', v_json->>'estimate_basis');
    ...
    r := r || public._qa_s13_ok('N4R65.B8 the quote carries the server floor ceiling',
          (v_json->>'min_ceiling_gnf')::bigint = 10000, v_json->>'min_ceiling_gnf');
    ...
    r := r || public._qa_s13_ok('N4R65.D12 the minimum ceiling rises to the estimated subtotal',
    ...
    r := r || public._qa_s13_ok('N4R65.D13 two observations are below the evidence gate',
    ...
    r := r || public._qa_s13_ok('N4R65.D14 one unpriced line makes the whole basket insufficient_data',
          v_j2->>'estimate_status' = 'insufficient_data', v_j2->>'estimate_status');
    r := r || public._qa_s13_ok('N4R65.D15 the priced line keeps its honest provenance inside a mixed basket',
          (SELECT count(*) FROM jsonb_array_elements(v_j2->'lines') l
            WHERE l->>'estimate_source' = 'observed_procurement') = 1, NULL);
    r := r || public._qa_s13_ok('N4R65.D16 the unpriced line is explicitly marked insufficient_data',
          (SELECT count(*) FROM jsonb_array_elements(v_j2->'lines') l
    ...
    BEGIN PERFORM public.marche_procurement_authorize(jsonb_build_object(
      ...
      v_err:='NO_ERROR'; EXCEPTION WHEN OTHERS THEN v_err := SQLERRM; END;
    r := r || public._qa_s13_ok('N4R65.E1 a ceiling below the server minimum is refused',
          v_err LIKE '%PROCUREMENT_CEILING_BELOW_MINIMUM%', v_err);
    BEGIN PERFORM public.marche_procurement_authorize(jsonb_build_object(
    ...
    r := r || public._qa_s13_ok('N4R65.F7 the audit trail is append-only',
          v_err LIKE '%PROCUREMENT_APPEND_ONLY%', v_err);
    v_err := public._qa_r6_err('authenticated', v_buy,
      $q$INSERT INTO public.marche_procurement_requests(buyer_user_id,authorized_ceiling_gnf,estimate_status,estimate_basis,estimate_unavailable_reason,line_count,item_count,client_request_id,request_fingerprint) VALUES (auth.uid(),1,'insufficient_data','customer_declared_ceiling','x',1,1,gen_random_uuid(),'x')$q$);
    ...
    BEGIN PERFORM public.marche_procurement_authorize(jsonb_build_object(
      'client_request_id', v_k1, 'ceiling_gnf', 60000, 'lines', v_b2));
      v_err:='NO_ERROR'; EXCEPTION WHEN OTHERS THEN v_err := SQLERRM; END;
    ...
    PERFORM set_config('request.jwt.claims', public._as_user_claims(v_buy), true);
    BEGIN PERFORM public.marche_procurement_increase(jsonb_build_object(
      'request_id', v_r1, 'client_request_id', gen_random_uuid(), 'new_ceiling_gnf', 40000));
      v_err:='NO_ERROR'; EXCEPTION WHEN OTHERS THEN v_err := SQLERRM; END;
    ...
    BEGIN PERFORM public.marche_procurement_settle_internal(v_r1, 1000, 'qa');
      v_err:='NO_ERROR'; EXCEPTION WHEN OTHERS THEN v_err := SQLERRM; END;
    r := r || public._qa_s13_ok('N4R65.J5 even with a customer JWT the settlement authority check refuses',
    ...
    r := r || public._qa_s13_ok('N4R65.J6 no refused settlement attempt changed any state', v_n = 0, v_n::text);
    ...
    v_json := public.marche_procurement_authorize(jsonb_build_object(
      'client_request_id', v_k4, 'ceiling_gnf', 20000, 'lines', v_b2));
    v_r3 := (v_json->>'id')::uuid;
    r := r || public._qa_s13_ok('N4R65.N1 a basket with no trustworthy estimate can still be authorized honestly',
          v_json->>'estimate_status' = 'insufficient_data'
      AND v_json->>'estimate_basis' = 'customer_declared_ceiling', v_json->>'estimate_basis');
    r := r || public._qa_s13_ok('N4R65.N2 no invented subtotal was frozen',
          v_json->'estimated_subtotal_gnf' = 'null'::jsonb, v_json->>'estimated_subtotal_gnf');
    r := r || public._qa_s13_ok('N4R65.N3 the mixed basket froze two immutable lines',
          (SELECT count(*) FROM public.marche_procurement_request_items WHERE request_id=v_r3) = 2, NULL);
    r := r || public._qa_s13_ok('N4R65.N4 the unit-native bunch line carries no fake normalization',
          EXISTS (SELECT 1 FROM public.marche_procurement_request_items
                  WHERE request_id=v_r3 AND option_code='botte' AND normalization_kind='unit_native'
                    AND canonical_base_unit IS NULL AND normalized_quantity IS NULL), NULL);
    PERFORM set_config('request.jwt.claims','',true);
    v_json := public.marche_procurement_settle_internal(v_r3, 20000, 'qa-exact');
    r := r || public._qa_s13_ok('N4R65.N5 spending exactly the ceiling captures the whole ceiling',
          (v_json->>'captured_gnf')::bigint = 20000, v_json->>'captured_gnf');
    r := r || public._qa_s13_ok('N4R65.N6 spending exactly the ceiling releases zero',
          (v_json->>'released_gnf')::bigint = 0, v_json->>'released_gnf');
    SELECT balance_gnf, held_gnf INTO v_bal, v_held FROM public.wallets
     WHERE owner_user_id=v_buy3 AND party_type='client';
    r := r || public._qa_s13_ok('N4R65.N7 the exact-ceiling customer paid exactly the ceiling',
          v_bal = 480000 AND v_held = 0, format('%s/%s', v_bal, v_held));
    ...
    r := r || public._qa_s13_ok('N4R65.Q7 the R6 catalog was not mutated by procurement', v_n = v_com0, v_n::text);
    SELECT count(*) INTO v_n FROM public.marche_staple_purchase_options;
    r := r || public._qa_s13_ok('N4R65.Q8 the R6 purchase options were not mutated by procurement', v_n = v_opt0, v_n::text);
    SELECT count(*) INTO v_n FROM public.marche_staple_variants;
    r := r || public._qa_s13_ok('N4R65.Q9 the R6 variants were not mutated by procurement', v_n = v_var0, v_n::text);
    SELECT count(*) INTO v_n FROM public.marche_staple_categories;
    r := r || public._qa_s13_ok('N4R65.Q10 the R6 categories were not mutated by procurement', v_n = v_cat0, v_n::text);
    SELECT COALESCE(sum(amount_gnf),0) INTO v_n FROM public.ledger_postings;
    r := r || public._qa_s13_ok('N4R65.Q11 the global ledger still sums to zero during the run', v_n = 0, v_n::text);
