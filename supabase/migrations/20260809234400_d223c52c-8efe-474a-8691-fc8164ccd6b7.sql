CREATE OR REPLACE FUNCTION public._qa_s3d_guards()
RETURNS jsonb
LANGUAGE sql
STABLE
SECURITY DEFINER
...
AS $$
  SELECT jsonb_build_object(
    'GUARD_ride_accept_auth_false', NOT has_function_privilege('authenticated','public.ride_accept(uuid)','execute'),
    'GUARD_ride_dispatch_anon_false', NOT has_function_privilege('anon','public.ride_dispatch(uuid)','execute'),
    'GUARD_ride_dispatch_auth_false', NOT has_function_privilege('authenticated','public.ride_dispatch(uuid)','execute'),
    'GUARD_internal_transfer_auth_false', NOT has_function_privilege('authenticated','public.wallet_internal_transfer(uuid,text,uuid,text,bigint,text,text)','execute')
  );
$$;
GRANT EXECUTE ON FUNCTION public._qa_s3d_guards() TO PUBLIC;
...
BEGIN
  ...
  UPDATE public.wallets SET balance_gnf = 0, held_gnf = 0, status='active' WHERE owner_user_id IN (u_d,u_c);
  ...
  VALUES ('orange_money','payment','QA-S3D-'||upper(substring(replace(gen_random_uuid()::text,'-',''),1,10)),'+224600000091',50000,'GNF','successful', jsonb_build_object('source','qa_sandbox'),'received', v_topup.customer_om_code_normalized, v_acct)
