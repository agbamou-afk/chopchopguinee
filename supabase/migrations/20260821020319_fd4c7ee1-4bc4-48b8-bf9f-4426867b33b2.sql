...
BEGIN
  ...
  r := r || public._qa_s13_ok('N5A5.A1 capability vocabulary function exists', v_caps IS NOT NULL, NULL);
  ...
  r := r || public._qa_s13_ok('N5A5.E4 refused direct write left the capability set untouched',
        (SELECT capabilities FROM public.driver_profiles WHERE user_id=u_d) = ARRAY['rides_moto'],
        (SELECT array_to_string(capabilities,',') FROM public.driver_profiles WHERE user_id=u_d));
  r := r || public._qa_s13_ok('N5A5.E5 the standard onboarding capability is itself canonical',
        'rides_moto' = ANY (public.driver_capability_vocabulary()), NULL);

  -- ============ F. RIGHT CLASS = CAPABILITY IS ASSIGNABLE ============
  PERFORM set_config('request.jwt.claims', public._as_user_claims(u_ops), true);
  PERFORM public.admin_set_driver_capability(u_d,'package_delivery',true);
  r := r || public._qa_s13_ok('N5A5.F1 capability is assignable to a DRIVER-class account',
        public.driver_capability_assigned(u_d,'package_delivery'), NULL);
  ...
  r := r || public._qa_s13_ok('N5A5.F8 approved DRIVER class + assignment = operational capability',
        public.driver_has_capability(u_d,'package_delivery') IS TRUE, NULL);
  r := r || public._qa_s13_ok('N5A5.F9 approval did not silently widen the capability set',
        public.driver_has_capability(u_d,'rides_taxi') IS FALSE, NULL);
  PERFORM set_config('request.jwt.claims', public._as_user_claims(u_ops), true);
  PERFORM public.admin_set_driver_capability(u_d,'package_delivery',true);
  r := r || public._qa_s13_ok('N5A5.F10 repeat grant is idempotent (no duplicate capability)',
        (SELECT count(*) FROM unnest((SELECT capabilities FROM public.driver_profiles WHERE user_id=u_d)) c
          WHERE c='package_delivery') = 1, NULL);
  ...
  PERFORM set_config('request.jwt.claims', public._as_user_claims(u_d), true);
  BEGIN PERFORM public.admin_set_driver_capability(u_d,'rides_taxi',true); v_err := NULL;
  EXCEPTION WHEN others THEN v_err := SQLERRM; END;
  r := r || public._qa_s13_ok('N5A5.G1 a driver cannot grant themselves a capability', v_err IS NOT NULL, v_err);
  r := r || public._qa_s13_ok('N5A5.G2 self-grant attempt changed nothing',
        public.driver_capability_assigned(u_d,'rides_taxi') IS FALSE, NULL);
  BEGIN PERFORM public.driver_set_capabilities(ARRAY['package_delivery','rides_taxi']); v_err := NULL;
  EXCEPTION WHEN others THEN v_err := SQLERRM; END;
  ...
  PERFORM public.driver_set_capabilities(ARRAY['package_delivery']);
  r := r || public._qa_s13_ok('N5A5.G4 a driver may narrow to already-granted capabilities',
        public.driver_has_capability(u_d,'package_delivery') IS TRUE, NULL);
  r := r || public._qa_s13_ok('N5A5.G5 narrowing dropped the capability the driver deselected',
  ...
  EXCEPTION WHEN others THEN v_err := SQLERRM; END;
  r := r || public._qa_s13_ok('N5A5.G6 approved driver with no grant cannot self-select',
        v_err ILIKE 'capability_not_granted%', v_err);
  PERFORM set_config('request.jwt.claims', public._as_user_claims(u_m), true);
  BEGIN PERFORM public.driver_set_capabilities(ARRAY['rides_moto']); v_err := NULL;
  EXCEPTION WHEN others THEN v_err := SQLERRM; END;
  r := r || public._qa_s13_ok('N5A5.G7 a MERCHANT cannot enter the driver capability surface',
        v_err = 'PROFESSIONAL_IDENTITY_CONFLICT', v_err);
  PERFORM set_config('request.jwt.claims', public._as_user_claims(u_c), true);
  BEGIN PERFORM public.driver_set_capabilities(ARRAY['rides_moto']); v_err := NULL;
  EXCEPTION WHEN others THEN v_err := SQLERRM; END;
  r := r || public._qa_s13_ok('N5A5.G8 a plain customer cannot enter the driver capability surface',
        v_err = 'PROFESSIONAL_IDENTITY_REQUIRED', v_err);
  PERFORM set_config('request.jwt.claims', NULL, true);
  BEGIN PERFORM public.driver_set_capabilities(ARRAY['rides_moto']); v_err := NULL;
  EXCEPTION WHEN others THEN v_err := SQLERRM; END;
  r := r || public._qa_s13_ok('N5A5.G9 unauthenticated capability selection is refused',
  ...
  r := r || public._qa_s13_ok('N5A5.H3 stored capability history is preserved, not erased',
        public.driver_capability_assigned(u_d,'package_delivery') IS TRUE, NULL);
  PERFORM set_config('request.jwt.claims', public._as_user_claims(u_ops), true);
  BEGIN PERFORM public.admin_set_driver_capability(u_d,'rides_taxi',true); v_err := NULL;
  EXCEPTION WHEN others THEN v_err := SQLERRM; END;
  r := r || public._qa_s13_ok('N5A5.H4 no capability can be granted after class release',
        v_err = 'PROFESSIONAL_IDENTITY_REQUIRED', v_err);
  PERFORM public.admin_set_driver_capability(u_d,'package_delivery',false);
  ...
  r := r || public._qa_s13_ok('N5A5.H7 refused bypass left capabilities empty',
        COALESCE(array_length((SELECT capabilities FROM public.driver_profiles WHERE user_id=u_d),1),0) = 0,
        (SELECT array_to_string(capabilities,',') FROM public.driver_profiles WHERE user_id=u_d));
  BEGIN
  ...
  r := r || public._qa_s13_ok('N5A5.I1 re-entry restores the DRIVER class',
        public.professional_active_type(u_d) = 'driver', NULL);
  r := r || public._qa_s13_ok('N5A5.I2 re-entry restores no revoked capability automatically',
        public.driver_capability_assigned(u_d,'package_delivery') IS FALSE, NULL);
