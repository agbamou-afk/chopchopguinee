...
BEGIN
  FOREACH f IN ARRAY ARRAY[
    ...
    '_qa_node3_repas_r8_discovery','_qa_node3_repas_r9_recovery_flows',
    ...
    agg := agg || jsonb_build_object('suite', f,
      'total', COALESCE(v->'total', jsonb_array_length(COALESCE(v->'results','[]'::jsonb))::text::jsonb),
      'failed', v->'failed',
      'failures', COALESCE(v->'failures',
        (SELECT COALESCE(jsonb_agg(y),'[]'::jsonb) FROM jsonb_array_elements(COALESCE(v->'results','[]'::jsonb)) y
          WHERE NOT (y->>'ok')::boolean)));
  END LOOP;
  INSERT INTO public._qa_s13_results(part, result) VALUES (9960, agg);
END $$;