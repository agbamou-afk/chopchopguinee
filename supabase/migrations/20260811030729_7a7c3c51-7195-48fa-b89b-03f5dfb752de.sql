CREATE TABLE IF NOT EXISTS public._qa_s9_results (
...
ALTER TABLE public._qa_s9_results ENABLE ROW LEVEL SECURITY;
REVOKE ALL ON public._qa_s9_results FROM anon, authenticated;
GRANT ALL ON public._qa_s9_results TO service_role;
GRANT ALL ON SEQUENCE public._qa_s9_results_id_seq TO service_role;
...
BEGIN
  ...
  BEGIN
    DECLARE
      ...
      v_n int; v_txt text; v_j uuid; v_sum bigint;
    ...
    BEGIN
      ...
      SELECT (SELECT balance_gnf FROM public.wallets WHERE id=v_w_c1) = b_c1 INTO v_txt;
      ...
      -- ============ D. GLOBAL PROVIDER REFERENCE UNIQUENESS ============
      BEGIN
        INSERT INTO public.payment_provider_events
        ...
        r := r || 'FAIL|D1 duplicate provider reference rejected|inserted';
      EXCEPTION WHEN unique_violation THEN
        r := r || 'PASS|D1 duplicate provider reference rejected|unique_violation';
      END;

      -- same reference claimed by a second customer
      INSERT INTO public.topup_requests
      ...
      BEGIN
        ...
        r := r || 'FAIL|D4 direct credit of consumed event to 2nd customer denied|credited';
      ...
      BEGIN
        ...
        r := r || 'FAIL|C6 forced credit on phone mismatch denied|credited';
      EXCEPTION WHEN OTHERS THEN
        r := r || format('PASS|C6 forced credit on phone mismatch denied|%s', SQLERRM);
      END;
      DELETE FROM public.payment_provider_events WHERE id=v_e3; DELETE FROM public.topup_requests WHERE id=v_tm;

      -- receiving account mismatch
      INSERT INTO public.topup_requests
        (reference, client_user_id, amount_gnf, confirmation_code, provider, status, expires_at,
         receiving_account_id, target_party_type, environment, customer_om_code_normalized, customer_om_code_submitted_at)
      ...
      BEGIN
        PERFORM public.wallet_topup_om_credit(v_e3, v_tm);
        r := r || 'FAIL|C8 forced credit on account mismatch denied|credited';
      EXCEPTION WHEN OTHERS THEN
      ...
      BEGIN
        ...
        r := r || 'FAIL|C12 forced credit on expired request denied|credited';
      EXCEPTION WHEN OTHERS THEN
      ...
      -- event with no matching request (honest waiting state)
      INSERT INTO public.payment_provider_events
        (provider, provider_transaction_id, amount_gnf, status, processing_status, receiving_account_id, environment)
      ...
      BEGIN
        ...
        r := r || 'FAIL|E3 forced sandbox->production credit denied|credited';
      EXCEPTION WHEN OTHERS THEN
        r := r || format('PASS|E3 forced sandbox->production credit denied|%s', SQLERRM);
      END;
      -- production event vs sandbox request
      UPDATE public.topup_requests SET environment='sandbox' WHERE id=v_tm;
      INSERT INTO public.payment_provider_events
      ...
      UPDATE public.topup_requests SET customer_om_code_normalized = public.normalize_om_code('PP260811.0309.E4') WHERE id=v_tm;
      v_res := public.om_auto_match(v_e3);
      ...
      BEGIN
        PERFORM public.wallet_topup_om_credit(v_e3, v_tm);
        r := r || 'FAIL|E5 forced production->sandbox credit denied|credited';
      EXCEPTION WHEN OTHERS THEN
        r := r || format('PASS|E5 forced production->sandbox credit denied|%s', SQLERRM);
      END;
      r := r || format('%s|E6 provider reference uniqueness is global (documented)|', CASE WHEN EXISTS (SELECT 1 FROM pg_indexes WHERE indexname='payment_provider_events_provider_tx_uidx') THEN 'PASS' ELSE 'FAIL' END);
      ...
      -- force rollback of every fixture / credit above
...
REVOKE ALL ON FUNCTION public._qa_s9_run() FROM PUBLIC, anon, authenticated;
GRANT EXECUTE ON FUNCTION public._qa_s9_run() TO service_role;