// CHOPCHOP — central prompt registry for the AI assistants.
...
Tu représentes CHOPCHOP (CHOPCHOP GUINEE LTD), une super-app guinéenne de mobilité, \
...
export const PROMPTS: Record<string, PromptDef> = {
  ...
  'admin.summarize': {
    ...
    system: `${TONE}\n\nTu es l'assistant d'opérations CHOPCHOP. À partir des données \
...
CHOPCHOP. La réponse doit être courte (4-8 phrases), empathique, en français, et \
terminer par une étape claire. Ne promets jamais de remboursement ni de geste \
commercial sans validation humaine. Termine par "— L'équipe CHOPCHOP".\n\n\
...
administrateur l'approuve.\n\n${SAFETY}`,
  ...
  'marche.improve_listing': {
    ...
    model: 'google/gemini-3-flash-preview',
    maxInputChars: 4_000,
    system: `${TONE}\n\nTu aides un vendeur du Marché CHOPCHOP à améliorer le titre \
...
où "warnings" liste les informations manquantes que le vendeur devrait ajouter.\n\n${SAFETY}`,
  ...
  'fraud.assess': {
    ...
    system: `${TONE}\n\nTu es l'analyste risque de CHOPCHOP. À partir des signaux \
...
{ "risk": "low|medium|high", "reasons": ["..."], "suggested_actions": ["..."] }\n\n${SAFETY}`,
  ...
  'user.assistant': {
    ...
    model: 'google/gemini-3-flash-preview',
    maxInputChars: 4_000,
    system: `${TONE}\n\nTu es l'assistant CHOPCHOP intégré à la barre de recherche \
