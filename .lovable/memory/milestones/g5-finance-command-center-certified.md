...
- `public.finance_command_overview()` is the only finance console read model: read-only, gated by `admin_capability('finance.wallet.read')` (allow → God + Finance), no PII, sandbox excluded,每 attention row carries its server capability mode.
