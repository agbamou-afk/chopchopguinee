...
/**
 * Finance / God admin surface for the Slice 4 dispute engine.
 * Resolution authority is enforced server-side by `_finance_privileged`.
 */
export function CashOrderDisputeQueue({ module }: { module?: "repas" | "marche" }) {
  const [rows, setRows] = useState<CashOrderRuntime[]>([]);
  ...
  const load = useCallback(async () => {
    ...
    setRows(error ? [] : ((data ?? []) as CashOrderRuntime[]));
    setLoading(false);
  }, [module]);

  useEffect(() => { void load(); }, [load]);

  const resolve = async (r: CashOrderRuntime, outcome: DisputeOutcome) => {
    setBusy(r.id);
    try {
      await resolveCashOrderDispute(r.source_module, r.source_id, outcome, note[r.id] || null);
  ...
  return (
    <section className="rounded-2xl border border-border/60 bg-card p-4 space-y-3">
      <h2 className="text-sm font-semibold inline-flex items-center gap-2">
        <Scale className="w-4 h-4 text-primary" /> Litiges commandes espèces
      </h2>
      {loading ? (
            ...
            <div className="flex flex-col gap-1.5">
              {OUTCOMES.map((o) => (
                <Button key={o} size="sm" variant={o === "complete_as_delivered" ? "default" : "outline"}
                  ...
                  {busy === r.id ? <Loader2 className="w-3 h-3 animate-spin" /> : DISPUTE_OUTCOME_LABEL[o]}
                </Button>
              ))}
            </div>
          </div>
